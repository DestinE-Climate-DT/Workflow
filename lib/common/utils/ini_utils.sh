...
function copy_restarts_from_expid() {
    ...
    # to the target restart directory of the new experiment.
    # The 'current' symlink in RESTARTS_FROM_PATH points to the latest completed chunk.
    # Arguments:
    #   $1  RESTART_DIR  destination directory. Callers pass the chunk-1 dir for a
    #       standard run (${PRE_RESTART_DIR}/1/) or the chunk-1/split-1 dir for an
    #       NRT run (${PRE_RESTART_DIR}/1/1/). Defaults to the standard layout.
    RESTART_DIR=${1:-${PRE_RESTART_DIR}/1/}
...
function create_inipath_symlinks_for_date_redirection() {
    ...
    for dir in ifsdata ifsINIT fesom tools; do
