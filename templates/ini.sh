...
NRT_ENABLED=${42:-%CONFIGURATION.NRT%}
...
if [ "${RESTARTED_RUN,,}" = "true" ]; then
    if [ "${NRT_ENABLED,,}" = "true" ]; then
        # NRT: copy into chunk-1/split-1 so the split-level hierarchy is correct
        # from the first split, then point 'current' there for RAPS.
        # lib/common/utils/ini_utils.sh (copy_restarts_from_expid) (auto generated comment)
        copy_restarts_from_expid "${PRE_RESTART_DIR}/1/1/"
        ln -sfn "1/1" "${PRE_RESTART_DIR}/current"
    else
        # lib/common/utils/ini_utils.sh (copy_restarts_from_expid) (auto generated comment)
        copy_restarts_from_expid "${PRE_RESTART_DIR}/1/"
    fi
fi
