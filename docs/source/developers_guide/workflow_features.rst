...
The perturbation seed for each member is computed as ``FESOM_PERTURBATION_SEED_BASE + realization``, and the realization follows the member name (fc0 → 50001, fc10 → 50011). This passes ``--fesom-perturb --fesom-perturbation-seed <seed>`` to RAPS.
...
Near-real-time (NRT) mode (IFS-FESOM)
--------------------------------------

In NRT mode the ``SIM`` job runs one simulation day at a time. Setting ``CONFIGURATION.NRT: "True"`` loads ``conf/toggles/nrt-True.yml``, which sets ``EXPERIMENT.SPLITSIZEUNIT: day`` and ``JOBS.SIM.SPLITS: auto`` so each chunk is divided into daily Autosubmit splits (one split = one simulation day), each depending on the previous one. Restart handling becomes split-aware: restarts are relocated per split and only the chunk-boundary restart is backed up.

For ``end-to-end`` runs, ``CONFIGURATION.DN`` (default ``"True"``) additionally loads ``conf/toggles/dn-True-nrt-True.yml``, which makes the ``DN`` job depend on the matching ``SIM`` split so it flows daily instead of waiting for the chunk's last split.

.. note::
   NRT is only supported for IFS-FESOM.

See :ref:`nrt` for how to enable it in an experiment.
