...
def normalize_tres_keys_and_units(
    ...
    for k, v in tres_dict.items():
        ...
        # GPU (gres/gpu*): clean names instead of the slash-mangled generic
        # ones. Check the typed util/mem keys before the bare gpu count.
        elif kl.startswith("gres/gpuutil"):
            out["Gpu_Utilization_Percent"] = convert_to_float(v, 0.0)
        elif kl.startswith("gres/gpumem"):
            out["Gpu_Memory_Bytes"] = convert_size_to_bytes(v)
        elif kl.startswith("gres/gpu"):
            out["Gpu_Count"] = convert_to_int(v, 0)
        else:
