...
def test_main_clmn_profile():
    ...
    assert (
        flat_request_content
        == "class=d1,dataset=climate-dt,experiment=ssp3-7.0,activity=projections,generation=2,model=ifs-nemo,realization=1,expver=0001,stream=clmn,resolution=high,type=fc,levtype=sfc,param=8/9/78/79/134/137/141/144/146/147/148/151/159/164/165/166/167/168/169/175/176/177/178/179/180/181/182/186/187/188/212/228/235/260048,month=1,year=2020"
    )
...
def test_main_clte_profile_omit_for_exec_wipe():
    ...
    tmp_dir = Path(tempfile.mkdtemp())
    general_request = (
        Path(__file__).parents[4] / "runscripts/FDB/general_request_clte.yaml"
    )
    flat_request_filename = str(tmp_dir / "test_request.flat")
    argv = [
        "--file",
    ...
    assert (
        flat_request_content
        == "class=d1,dataset=climate-dt,activity=projections,experiment=ssp3-7.0,generation=2,model=ifs-nemo,realization=1,expver=0001,stream=clte,date=,type=fc"
    )
...
def test_main_clmn_profile_omit_for_exec_wipe():
    ...
    tmp_dir = Path(tempfile.mkdtemp())
    general_request = (
        Path(__file__).parents[4] / "runscripts/FDB/general_request_clmn.yaml"
    )
    flat_request_filename = str(tmp_dir / "test_request.flat")
    argv = [
        "--file",
    ...
    assert (
        flat_request_content
        == "class=d1,dataset=climate-dt,activity=projections,experiment=ssp3-7.0,generation=2,model=ifs-nemo,realization=1,expver=0001,stream=clmn,type=fc,month=1,year=2020"
    )
...
def test_main_clmn_profile_omit_for_purge():
    ...
    assert (
        flat_request_content
        == "class=d1,dataset=climate-dt,experiment=ssp3-7.0,activity=projections,generation=2,model=ifs-nemo,realization=1,expver=0001,stream=clmn,year=2020"
    )


def test_pl_with_omit_levelist():
    tmp_dir = Path(tempfile.mkdtemp())
    profile_file = Path(__file__).parent / "pl_hourly_healpix_high.yaml"
    flat_request_filename = str(tmp_dir / "test_request.flat")
    argv = [
        "--file",
        str(profile_file),
    ...
    assert (
        flat_request_content
        == "class=d1,dataset=climate-dt,experiment=ssp3-7.0,activity=projections,generation=2,model=ifs-nemo,realization=1,expver=0001,stream=clte,date=,resolution=high,type=fc,levtype=pl,time=0000/0100/0200/0300/0400/0500/0600/0700/0800/0900/1000/1100/1200/1300/1400/1500/1600/1700/1800/1900/2000/2100/2200/2300,param=60/129/130/131/132/133/135/157/246"
    )


def test_pl_no_omit_levelist():
    tmp_dir = Path(tempfile.mkdtemp())
    profile_file = Path(__file__).parent / "pl_hourly_healpix_high.yaml"
    flat_request_filename = str(tmp_dir / "test_request.flat")
    argv = [
