...
| DATA_PORTFOLIO                   | `string` |  | `full` `reduced` |  |  | Used to determine the set of variables in the data portfolio.                                                 |                                |
...
| IFS                              | `object` |  | object |  |  |                                                                                                               |                                |
| VARS_TO_EXPORT                   | `string` |  | string |  |  | If one or more variables separated by a comma are entered, they will be exported during the IFS-NEMO SIM job. | LD_LIBRARY_PATH SIM_START_DATE |
