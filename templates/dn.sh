...
FDB_LIST_BIN=${47:-%CURRENT_FDB_LIST_BIN%}
SPLIT_START_DATE=${48:-%SPLIT_START_DATE%}
SPLIT_SECOND_TO_LAST_DATE=${49:-%SPLIT_SECOND_TO_LAST_DATE%}
EXPERIMENT=${50:-%REQUEST.EXPERIMENT%}
ACTIVITY=${51:-%REQUEST.ACTIVITY%}
MODEL_NAME=${52:-%REQUEST.MODEL_NAME%}
GENERATION=${53:-%REQUEST.GENERATION%}
...
function run_DN() {
    ...
    singularity exec --cleanenv --no-home --bind "${SCRIPTDIR}/dn/" --bind "${FDB_HOME}" --bind "${DEVELOPMENT_PROJECT_SCRATCH}" --bind "${OPERATIONAL_PROJECT_SCRATCH}" --bind "${LOGDIR}/" --bind "${REQUESTDIR}"/ --env FDB_HOME=${FDB_HOME} --env request_dir="${REQUESTDIR}" --env chunk="$1" --env split="$2" --env expid="$3" --env app_names="$4" --env run_type="$5" --env member="$6" --env WORKFLOW="$7" --env datelist="$8" --env "PYTHONNOUSERSITE=1" --env LOGDIR="${LOGDIR}" ${HPC_CONTAINER_DIR}/gsv/gsv_${GSV_VERSION}.sif bash -c '
        python3 run_dn.py \
