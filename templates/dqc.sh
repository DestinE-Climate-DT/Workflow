...
function run_negative_space_checker() {
	...
	sample_file_clte=$(find ${DQC_PROFILE_PATH} -maxdepth 1 -type f -name "*.yaml" ! -name "*monthly*" | sort | head -n1)
        sample_file_clmn=$(find ${DQC_PROFILE_PATH} -maxdepth 1 -type f -name "*monthly*.yaml" | sort | head -n1)
	...
	for levtype in sfc pl o2d o3d hl sol; do
		...
		FLAT_REQ_NAME="${NEGATIVE_CHECK_DIR}/${levtype}_clte.flat"
                python3 "${SCRIPTDIR}/FDB/yaml_to_flat_request.py" --file="${sample_file_clte}" \
                  ...
                  --omit-keys "time,levelist,param,resolution" --request_name="${FLAT_REQ_NAME}"
...
ADDITIONAL_BINDINGS=("${FDB_INFO_FILE_PATH}" "${DQC_OUTPUT_DIR}" "$(realpath ${FDB_HOME})")
