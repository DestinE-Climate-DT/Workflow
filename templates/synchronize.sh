...
TRANSFER_ON=${13:-%CONFIGURATION.ADDITIONAL_JOBS.TRANSFER%}
WIPE_ON=${14:-%CONFIGURATION.ADDITIONAL_JOBS.WIPE%}
...
if [ ! -f ${flag_tarball_path} ]; then
    ...
    # If the tarball was already sent, we can just rsync the project directory
    rsync_to_remote "${HPCUSER}" "${HPCHOST}" "${PROJDEST}" "${HPCROOTDIR}"
fi

if [ "${TRANSFER_ON,,}" == "true" ] || [ "${WIPE_ON,,}" == "true" ]; then
