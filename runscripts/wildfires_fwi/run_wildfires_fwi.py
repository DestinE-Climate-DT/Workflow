...
if (month_in == 12 and day_in == 31) or (
    month_in == 12 and day_in == 30 and not calendar.isleap(year_in)
):
    ...
    if len(file_list) == expected_days:
        ds_combined = xr.open_mfdataset(
            file_list, combine="by_coords", chunks={"time": 30}
        ).chunk({"time": -1})

        # Mean, Max, Min
        ds_combined.fwi.mean(dim="time").to_dataset(name="fwi").to_netcdf(
            os.path.join(OUTPUT_DIR, f"{year_in}_fwi_mean.nc"), compute=False
        ).compute()
        ds_combined.fwi.max(dim="time").to_dataset(name="fwi").to_netcdf(
            os.path.join(OUTPUT_DIR, f"{year_in}_fwi_max.nc"), compute=False
        ).compute()
        ds_combined.fwi.min(dim="time").to_dataset(name="fwi").to_netcdf(
            os.path.join(OUTPUT_DIR, f"{year_in}_fwi_min.nc"), compute=False
        ).compute()

        # Percentiles
        for perc in [50, 95, 99]:
            percentile = ds_combined.fwi.quantile(perc / 100, dim="time")
            percentile.to_dataset(name="fwi").to_netcdf(
                os.path.join(OUTPUT_DIR, f"{year_in}_fwi_{perc}per.nc"), compute=False
            ).compute()

        # DSR Monthly
        dsr = 0.0272 * np.power(ds_combined.fwi, 1.77)
        dsr.resample(time="1M").mean().to_dataset(name="fwi").to_netcdf(
            os.path.join(OUTPUT_DIR, f"{year_in}_fwi_DSR_monthly.nc"), compute=False
        ).compute()
        ...
        for label, mask in thresholds.items():
            mask.sum(dim="time").to_dataset(name="fwi").to_netcdf(
                os.path.join(OUTPUT_DIR, f"{year_in}_fwi_days_{label}.nc"),
                compute=False,
            ).compute()
    ...
    print(
        f"Not the last day of the year ({year_in}-{month_in:02d}-{day_in:02d}). Skipping yearly stats."
    )
