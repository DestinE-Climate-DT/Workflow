...
class AppExperiment:
    ...
    Attributes:
        chunk (int): Input chunk number for data notifier.
        split (int): Input split number for data notifier.
        expid (str): Experiment ID.
        datelist (str): Simulation start date(s).
        member (Optional[str]): Member name (default: "").
    ...
    expid: str
    datelist: str
    member: T.Optional[str] = ""
    realization: T.Optional[int] = None
    ...
    def get_request_files(self, directory):
        ...
        member_or_realization = (
            f"_{self.member}"
            if self.member
            else f"_{self.realization}"
            if self.realization
            else ""
        )
        file_name = (
            f"{prefix}*{member_or_realization}_{self.chunk}_{self.split}{suffix}"
        )
        ...
        if self.app_names:
            request_files = list(
                filter(
                    lambda x: any(app_name.lower() in x for app_name in self.app_names),
                    request_files,
                )
            )
...
def add_app_experiment_arguments(parser):
    ...
    group.add_argument(
        "--datelist", type=str, nargs="*", help="Simulation start date(s)"
    )
    return parser
