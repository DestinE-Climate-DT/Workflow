...
CATALOG=${8:-%CURRENT_CATALOG_NAME%}
...
CARTOPY_DATA_DIR=${16:-%AQUA.CARTOPY_DATA_DIR%}
EXPERIMENT_NAME=${17:-%AQUA.EXPERIMENT_NAME%}
...
singularity exec \
    ...
    bash -c \
    "
    python $AQUA/cli/aqua-analysis/aqua-analysis.py -l debug -m $MODEL -e $EXPERIMENT_NAME -s $SOURCE -d $OUTPATH -c $CATALOG --parallel
    "
...
if find "${OUTPATH}/${CATALOG}/${MODEL}/${EXPERIMENT_NAME}" -type f -name "*.pdf" -mmin -5 | grep -q .; then
