...
- pie.csv: Component timing breakdown including IFS, FESOM, and I/O

Note: FESOM I/O timing currently extracted from pie.csv total FESOM time.
Detailed separation between FESOM compute and I/O requires implementing
timers in the FESOM interface with MultIO.
"""
...
class IFSFESOMParser(ModelParser):
    ...
    def parse(self) -> IOMetrics:
        ...
        # Add notes about allocation mode, FESOM I/O, and shared compute tasks
        notes = resource_metrics.get("allocation_notes", [])
        if not isinstance(notes, list):
            notes = []
        notes.append(
            "FESOM I/O time not separately available - included in total FESOM time from pie.csv"
        )
        ...
        time_metrics["compute_time"] = ifs_compute_time + fesom_total_time
    ...
    def _extract_time_metrics(self) -> Dict[str, float]:
        ...
        Time calculation:
        - IFS time = sum(all components) - "I/O" - "FESOM" (from pie.csv)
        - IFS I/O time = "I/O" component (from pie.csv)
        - FESOM total time = "FESOM" component (from pie.csv)
        - FESOM I/O time = 0.0 (not separately available)
        ...
        Note:
            FESOM I/O time is not separately available and would require
            implementing timers in the FESOM interface with MultIO.
        ...
        fesom_total_time = component_times.get("FESOM", 0.0)
        total_time = sum(component_times.values())

        # Calculate IFS time: all components except I/O and FESOM
        ...
        if ifs_time < 0:
            raise ValueError(
                ...
                f"FESOM: {fesom_total_time:.2f}s"
            )

        # FESOM I/O time is not separately available
        # It's included in the total FESOM time from pie.csv
        fesom_io_time = 0.0
