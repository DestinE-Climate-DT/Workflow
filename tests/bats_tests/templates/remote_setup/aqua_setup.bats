...
setup() {
    ...
    # fake it or the real install_aqua will be called
    ...
    export CATALOG_NAME="mn5-phase2"
    export EXPID="a000"
    export EXPVER="test"
...
@test "AQUA False" {
    AQUA_ON="False"
    run source "${HPCROOTDIR}/proj/${PROJDEST}/templates/remote_setup/aqua_setup.sh" ${CURRENT_ARCH} ${LIBDIR} ${AQUA_ON} ${HPC_CONTAINER_DIR} ${CONTAINER_VERSION} ${SCRATCH_DIR} ${HPC_PROJECT_ROOT} ${LOCAL_DIR} ${HPCROOTDIR} ${AQUA_REGENCAT} ${HPCARCH_short} ${PROJDEST} ${CATALOG_NAME} ${EXPID} ${EXPVER} ${MODEL} ${DATA_PORTFOLIO} ${SIM_START_DATE} ${USER}
