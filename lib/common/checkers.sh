...
function checker_run_type() {
    ...
    allowed_run_types=("production" "research" "test" "pre-production" "operational" "operational-read")

    valid_run_type=false
    for allowed_type in "${allowed_run_types[@]}"; do
        if [[ "${RUN_TYPE}" == "${allowed_type}" ]]; then
            valid_run_type=true
            break
        fi
    done

    if [[ "${valid_run_type}" == true ]]; then
        echo "RUN_TYPE is ${RUN_TYPE}"
    else
        echo "RUN_TYPE is not valid. Please choose between ${allowed_run_types[*]}"
        exit 1
    fi
}
