...
For more information and to gain access please follow `these instructions <https://wiki.eduuni.fi/spaces/cscRDIcollaboration/pages/462100843/Autosubmit+Virtual+Machine>`_. You can also find additional information in the section :ref:`autosubmit` of this documentation.
...
The data can be automatically transferred to the Data Bridge using the TRANSFER jobs. Once it is correctly transferred, it can then be deleted from the HPC using the WIPE jobs, which first validate the transfer (``WIPE_CHECK``) and then execute the deletion (``WIPE``). See :ref:`jobs` for configuration details.
