...
setup() {
    ...
    export DATA_DIR="${ROOTDIR}/data"
    export HPC_CONTAINER_DIR="${ROOTDIR}/container"
    export OPERATIONAL_PROJECT_SCRATCH="${HPC_SCRATCH}/operational"
    export DEVELOPMENT_PROJECT_SCRATCH="${HPC_SCRATCH}/development"
    export CARTOPY_DATA_DIR="${HPC_CONTAINER_DIR}/cartopy_data"
    export EXPERIMENT_NAME=${EXPID}

}

@test "load_template_without_mocked_pdfs" {
    run source "${ROOTDIR}/proj/${PROJDEST}/templates/aqua/aqua_analysis.sh" "${ROOTDIR}" "${PROJDEST}" "${EXPID}" "${HPC_PROJECT}" "${CONTAINER_VERSION}" "${MODEL}" "${EXPVER}" "${CATALOG_NAME}" "${APP_OUTPATH}" "${CURRENT_ARCH}" "${HPC_SCRATCH}" "${DATA_DIR}" "${HPC_CONTAINER_DIR}" "${OPERATIONAL_PROJECT_SCRATCH}" "${DEVELOPMENT_PROJECT_SCRATCH}" "${CARTOPY_DATA_DIR}" "${EXPERIMENT_NAME}"
    assert_failure
}

@test "load_template_with_mocked_pdfs" {
    mock_pdfs ${APP_OUTPATH} ${CATALOG_NAME} ${MODEL} ${EXPERIMENT_NAME}
    run source "${ROOTDIR}/proj/${PROJDEST}/templates/aqua/aqua_analysis.sh" "${ROOTDIR}" "${PROJDEST}" "${EXPID}" "${HPC_PROJECT}" "${CONTAINER_VERSION}" "${MODEL}" "${EXPVER}" "${CATALOG_NAME}" "${APP_OUTPATH}" "${CURRENT_ARCH}" "${HPC_SCRATCH}" "${DATA_DIR}" "${HPC_CONTAINER_DIR}" "${OPERATIONAL_PROJECT_SCRATCH}" "${DEVELOPMENT_PROJECT_SCRATCH}" "${CARTOPY_DATA_DIR}" "${EXPERIMENT_NAME}"
    assert_success
}
