...
setup() {
    ...
    export REALIZATION="2"  # expected output of get_member_number "${MEMBER_LIST}" ${MEMBER}
    export EXPVER="a000"
    export STREAM="clte"
...
@test "load_template_empty_fdb" {
    setup_fdb
    run source "${ROOTDIR}/proj/${PROJDEST}/templates/performance/check_size.sh" \
        "${CLASS}" "${DATASET}" "${ACTIVITY}" "${EXPERIMENT}" "${GENERATION}" "${MODEL}" \
        "${MEMBER_LIST}" "${EXPVER}" "${STREAM}" "${START_DATE}" "${END_DATE}" \
        "${FDB_HOME}" "${HPCROOTDIR}" "${MEMBER}" "${LIBDIR}" "${CURRENT_ARCH}"
    assert_failure
}
