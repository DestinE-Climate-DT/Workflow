...
class Logger(logging.Logger):
    ...
    def dashed_line(self, verbose=0):
        ...
        """
        if verbose <= get_global_verbose():
            # set formater to none log with info and restore the format
            formatter = self.handlers[0].formatter
            self.handlers[0].setFormatter(None)
            self.info("-" * 40)
...
# Global logger
__logger = logging.Logger("wf_mock")
