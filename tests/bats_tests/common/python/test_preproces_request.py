...
def test_reading_file():
    ...
    example_request = root / "conf" / "applications" / "request" / "request_ENERGY_INDICATORS.yml"
    output_dir = Path(tempfile.mkdtemp())
    jobname = "TEST"
    main(argv=["--file", str(example_request), "--output-dir", str(output_dir), "--omit-keys", "grid,method,area", "--jobname", jobname])
    ...
    for file in output_dir.iterdir():
        ...
        for key in MARS_KEYS:
            assert key in profile["mars-keys"], f"Key {key} not found in mars-keys of {file.name}"

        if profile["mars-keys"]["stream"] == "clte":
            assert profile["date-format"] == "date", f"Expected date-format 'date' for stream 'clte' in {file.name}"
        else:
            assert profile["date-format"] == "month", f"Expected date-format 'month' for stream '{profile['mars-keys']['stream']}' in {file.name}"

        for key in {"grid", "method", "area"}:
            assert key not in profile["mars-keys"], f"Key {key} should be omitted from mars-keys in {file.name}"
...
def test_preprocess_request_derived_variables():
    ...
    output_dir = Path(tempfile.mkdtemp())
    jobname = "TEST"
    main(argv=["--file", str(example_request), "--output-dir", str(output_dir), "--omit-keys", "grid,method,area", "--jobname", jobname, "--process-derived-variables"])

    expected_variables = ['u', 'v', '10si', 'avg_sdswrf', 'ws', '2t']
    # Map from shortname to paramId, with derived variables already substituted.
    expected_params = {
        "u": ["131"],  # GSV processed request always returns param as a list of strings.
        "v": ["132"],
        "10si": ["207"],
        "ws": ["131", "132"],
        "2t": ["167"],
    ...
    for file in output_dir.iterdir():
        ...
        variable = file.name.removesuffix(f"_{jobname}.yaml")  # Only valid for Python 3.9+
