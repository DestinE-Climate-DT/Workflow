...
@test "load_template" {
    run source "${ROOTDIR}/proj/${PROJDEST}/templates/aqua/aqua_push.sh" "${HPCROOTDIR}" "${ROOTDIR}" "${HPCUSER}" "${HPCHOST}" "${PROJDEST}" "${MODEL}" "${EXPVER}" "${APP_OUTPATH}" "${HPCARCH}" "${CATALOG_NAME}" "${BUCKET}" "${MEMBER}" "${MEMBER_LIST}"
    assert_success
}
