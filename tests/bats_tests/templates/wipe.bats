...
setup() {
    ...
    export BRIDGE_EXPVER="test"
    export MODIFY_METADATA_TRANSFER="false"
    export MODIFY_METADATA_FIELDS="false"
...
@test "load_template_with_doit" {
    WIPE_DOIT="true"
    run source "${ROOTDIR}/proj/${PROJDEST}/templates/wipe.sh" ${ROOTDIR} ${CURRENT_ARCH} ${CHUNK} ${START_DATE} ${SECOND_TO_LAST_DATE} ${MODEL_NAME} ${DATABRIDGE_FDB_HOME} ${EXPERIMENT} ${ACTIVITY} ${GENERATION} ${DQC_PROFILE_PATH} ${EXPVER} ${FDB_HOME} ${SCRATCH_DIR} ${HPC_CONTAINER_DIR} ${GSV_VERSION} ${LIBDIR} ${SCRIPTDIR} ${WIPE_DOIT} ${MEMBER} "${MEMBER_LIST}" ${FDB_INFO_FILE_PATH} ${FDB_INFO_FILE_NAME} ${BASE_VERSION} ${SPLIT_END_DATE} ${OPERATIONAL_PROJECT_SCRATCH} ${DEVELOPMENT_PROJECT_SCRATCH} ${BRIDGE_EXPVER} ${MODIFY_METADATA_TRANSFER} ${MODIFY_METADATA_FIELDS}
    assert_success
}

@test "single_member_advances_data_start_date" {
    WIPE_DOIT="true"
    MEMBER="fc0"
    MEMBER_LIST="fc0"
    run source "${ROOTDIR}/proj/${PROJDEST}/templates/wipe.sh" \
        ${ROOTDIR} ${CURRENT_ARCH} ${CHUNK} ${START_DATE} ${SECOND_TO_LAST_DATE} \
        ${MODEL_NAME} ${DATABRIDGE_FDB_HOME} ${EXPERIMENT} ${ACTIVITY} \
        ${GENERATION} ${DQC_PROFILE_PATH} ${EXPVER} ${FDB_HOME} ${SCRATCH_DIR} \
        ${HPC_CONTAINER_DIR} ${GSV_VERSION} ${LIBDIR} ${SCRIPTDIR} ${WIPE_DOIT} \
        ${MEMBER} "${MEMBER_LIST}" ${FDB_INFO_FILE_PATH} ${FDB_INFO_FILE_NAME} \
        ${BASE_VERSION} ${SPLIT_END_DATE} ${OPERATIONAL_PROJECT_SCRATCH} \
        ${DEVELOPMENT_PROJECT_SCRATCH} ${BRIDGE_EXPVER} \
        ${MODIFY_METADATA_TRANSFER} ${MODIFY_METADATA_FIELDS}
    assert_success
    assert [ -f "${ROOTDIR}/wipe_done/${CHUNK}/r1" ]
}
...
@test "load_template_with_change_metadata" {
    WIPE_DOIT="true"
    MODIFY_METADATA_TRANSFER="true"
    MODIFY_METADATA_FIELDS="experimentVersionNumber=0001"
    BRIDGE_EXPVER="0001"
    run source "${ROOTDIR}/proj/${PROJDEST}/templates/wipe.sh" ${ROOTDIR} ${CURRENT_ARCH} ${CHUNK} ${START_DATE} ${SECOND_TO_LAST_DATE} ${MODEL_NAME} ${DATABRIDGE_FDB_HOME} ${EXPERIMENT} ${ACTIVITY} ${GENERATION} ${DQC_PROFILE_PATH} ${EXPVER} ${FDB_HOME} ${SCRATCH_DIR} ${HPC_CONTAINER_DIR} ${GSV_VERSION} ${LIBDIR} ${SCRIPTDIR} ${WIPE_DOIT} ${MEMBER} "${MEMBER_LIST}" ${FDB_INFO_FILE_PATH} ${FDB_INFO_FILE_NAME} ${BASE_VERSION} ${SPLIT_END_DATE} ${OPERATIONAL_PROJECT_SCRATCH} ${DEVELOPMENT_PROJECT_SCRATCH} ${BRIDGE_EXPVER} ${MODIFY_METADATA_TRANSFER} ${MODIFY_METADATA_FIELDS}
    assert_success
}
...
@test "load_template_with_not_matching_metadata" {
    WIPE_DOIT="true"
    MODIFY_METADATA_TRANSFER="true"
    MODIFY_METADATA_FIELDS="experimentVersionNumber=0001"
    BRIDGE_EXPVER="a001"
