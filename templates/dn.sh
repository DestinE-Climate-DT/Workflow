...
HPC_CONTAINER_DIR=${21:-%CURRENT_CONTAINER_DIR%}
...
APP_HYDROLAND=${31:-%APP.HYDROLAND%}
APP_HYDROMET=${32:-%APP.HYDROMET%}
APP_OBSALL=${33:-%APP.OBSALL%}
APP_WILDFIRES_FWI=${34:-%APP.WILDFIRES_FWI%}
APP_WILDFIRES_WISE=${35:-%APP.WILDFIRES_WISE%}
APP_DATA=${36:-%APP.DATA%}
HPC_PROJECT_ROOT=${37:-%CURRENT_HPC_PROJECT_ROOT%}
LOCAL_DIR=${38:-%CURRENT_LOCAL_DIR%}
FDB_CONFIG_GATEWAY=${39:-%CONFIGURATION.FDB_CONFIG_GATEWAY%}
JOBNAME=${40:-%JOBNAME%}
CONTAINER_COMMAND=${41:-%CURRENT_CONTAINER_COMMAND%}
CURRENT_ROOTDIR=${42:-%CURRENT_ROOTDIR%}
FDB_COPY_BIN=${43:-%CURRENT_FDB_COPY_BIN%}
FDB_CONFIG_DATABRIDGE=${44:-%CONFIGURATION.FDB_CONFIG_DATABRIDGE%}
FDB_CONFIG_HPC=${45:-%CONFIGURATION.FDB_CONFIG_HPC%}
FDB_LIST_BIN=${46:-%CURRENT_FDB_LIST_BIN%}
SPLIT_START_DATE=${47:-%SPLIT_START_DATE%}
SPLIT_SECOND_TO_LAST_DATE=${48:-%SPLIT_SECOND_TO_LAST_DATE%}
EXPERIMENT=${49:-%REQUEST.EXPERIMENT%}
ACTIVITY=${50:-%REQUEST.ACTIVITY%}
MODEL_NAME=${51:-%REQUEST.MODEL_NAME%}
GENERATION=${52:-%REQUEST.GENERATION%}
REQUEST_REALIZATION=${53:-%REQUEST.REALIZATION%}
...
for var in APP_ENERGY_INDICATORS APP_HYDROLAND APP_HYDROMET APP_OBSALL APP_WILDFIRES_FWI APP_WILDFIRES_WISE APP_DATA; do
    value="${!var}"
    if [[ "$value" == "True" ]]; then
...
source "${LIBDIR}"/common/utils/dn_utils.sh
...
# convert mamber list to an actual array
...
#####################################################
# Read from databridge marenostrum
# run_DN
#   Runs Data Notifier.
# Globals:
#   SCRIPTDIR
...
######################################################
function run_DN() {
    # lib/common/utils/dn_utils.sh (print_data_gov) (auto generated comment)
    print_data_gov
...
if [ "${READ_FROM_DATABRIDGE,,}" = "true" ] && [ "$CURRENT_ARCH" = "${HPC}-READDATABRIDGE" ]; then
    source "$LIBDIR/${HPC}-TRANSFER/config.sh"
    echo "run_DN was overridden by the run_DN to read from MN5 Databridge"
fi
