...
Set applications gsvrequest details (resolution, area selection and interpolation method):
-----------------------------------------
The area, resolution and interpolation method (soon to come) of the data from the streaming (the input data to the applications) can be defined without having to modify the file `conf/mother_request.yml`.

The defaults are set under `conf/applilcations/gsvrequest.yml`.
