...
HPC_CONTAINER_DIR=${38:-%CONFIGURATION.CONTAINER_DIR%}
...
SPLITS=${46:-%SPLITS%}
ENERGY_INDICATORS_IN_DATA_VERSION=${47:-%ENERGY_INDICATORS.IN_DATA_VERSION%}
ENERGY_OFFSHORE_IN_DATA_VERSION=${48:-%ENERGY_OFFSHORE.IN_DATA_VERSION%}
...
HYDROLAND_SPLITSIZEUNIT=${61:-%EXPERIMENT.SPLITSIZEUNIT%}
...
function run_ENERGY_INDICATORS() {
    ...
    mkdir -p "${APP_OUTPATH}/energy_indicators/${SPLIT_INI_YEAR}/${SPLIT_INI_MONTH}/"

    # copy histograms as if they were APP output
    FILE=${APP_OUTPATH}/energy_indicators/opa/*histogram_bin*
    if ls $FILE 1>/dev/null 2>&1; then
        mv $FILE ${APP_OUTPATH}/energy_indicators/${SPLIT_INI_YEAR}/${SPLIT_INI_MONTH}/
    ...
    singularity exec \
        ...
        --bind "${HPC_SCRATCH}" \
        --bind "${HPC_PROJECT}" \
        --bind "${APP_OUTPATH}/energy_indicators/opa/" \
        --bind "${APP_OUTPATH}/energy_indicators/" \
        --bind "${PYTHONPATH}" \
        ...
        bash -c \
        "
    python3 "${SCRIPTDIR}"/energy_indicators/run_energy_indicators.py --iniyear ${SPLIT_INI_YEAR} --inimonth ${SPLIT_INI_MONTH} --iniday ${SPLIT_INI_DAY} --in_path "${APP_OUTPATH}/energy_indicators/opa/" --finyear ${SPLIT_INI_YEAR} --finmonth ${SPLIT_INI_MONTH} --finday ${SPLIT_INI_DAY} --out_path "${APP_OUTPATH}/energy_indicators/${SPLIT_INI_YEAR}/${SPLIT_INI_MONTH}/"
    "
}
...
function run_HYDROLAND() {
    ...
    singularity exec \
        ...
        --no-home \
        --bind "${HPC_PROJECT}" \
        --bind "${APP_OUTPATH}/hydroland/opa" \
        ...
        --hydroland-opa "${APP_OUTPATH}/hydroland/opa" \
