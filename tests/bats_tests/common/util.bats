...
@test "get_member_number fails for a member name outside the fcN format or different than default." {
	run get_member_number "random" "random"
	assert_failure
	assert_output "Member 'random' is not in the expected fcN format or different than default."
}

# "default" was ICON's member name before v6.2.4 and still maps to realization 1
@test "get_member_number maps the legacy ICON member name to realization 1" {
	assert_equal "$(get_member_number "default" "default")" "1"
	assert_equal "$(get_realization_list "default")" "1"
}
...
@test "get_member_number writes errors to stderr, not into the captured value" {
	CAPTURED=$(get_member_number "random" "random" 2>/dev/null) || true
...
@test "get_realization_list fails on a member outside the fcN format" {
	run get_realization_list "fc0 random"
	assert_failure

	CAPTURED=$(get_realization_list "fc0 random" 2>/dev/null) || true
	assert_equal "${CAPTURED}" ""
}
