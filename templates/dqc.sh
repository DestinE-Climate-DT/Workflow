...
SPLIT_END_DATE=${37:-%SPLIT_END_DATE%}
UPDATE_FDB_INFO=${38:-%CURRENT_UPDATE_FDB_INFO%}
SPLIT_START_DATE=${39:-%SPLIT_START_DATE%}
SPLIT_SECOND_TO_LAST_DATE=${40:-%SPLIT_SECOND_TO_LAST_DATE%}
...
function run_DQC_prod() {
    ...
    singularity exec --cleanenv --no-home \
        ...
        --dqc_output_file "${DQC_OUTPUT_FILE}" ${DQC_PROFILES:+--profiles "${DQC_PROFILES}"} '
}
...
function run_negative_space_checker() {
	...
	NEGATIVE_CHECK_DIR="${DQC_OUTPUT_DIR}/negative_check_files/sdate_${CHUNK_START_DATE}_endate_${CHUNK_SECOND_TO_LAST_DATE}_real_${REALIZATION}"
...
# Compute Date key. Split jobs (NRT) check only their own day(s); the SPLIT_*
# placeholders resolve to empty strings for jobs without splits, in which case
# the whole chunk is checked as before.
if [ -n "${SPLIT_START_DATE}" ]; then
    DATE="${SPLIT_START_DATE}/to/${SPLIT_SECOND_TO_LAST_DATE}"
else
...
# Split jobs (NRT) check single days, so the pass above skips the monthly
# (clmn) profiles (gsv allow_submonthly_chunks). The monthly means only reach
# the FDB once the month is complete: when this split crosses the month
# boundary, re-run the DQC for the monthly profiles over the whole month.
if [[ "${SPLIT_START_DATE}" =~ ^[0-9]{8}$ ]]; then
    # lib/common/util.sh (requires_process_monthly_nrt) (auto generated comment)
    DQC_MONTHLY=$(requires_process_monthly_nrt "${SPLIT_START_DATE}" "${SPLIT_END_DATE}")
    if [ "${DQC_MONTHLY}" == "true" ]; then
        MONTHLY_PROFILES=$(find "${DQC_PROFILE_PATH}" -maxdepth 1 -name "*monthly*.yaml" -printf "%f\n" | paste -sd, -)
        if [ -n "${MONTHLY_PROFILES}" ]; then
            MONTH_START_DATE=$(date -d "${SPLIT_START_DATE}" +'%Y%m01')
            MONTH_DATE="${MONTH_START_DATE}/to/${SPLIT_SECOND_TO_LAST_DATE}"
            DQC_MONTHLY_OUTPUT_FILE="${DQC_OUTPUT_DIR}/${JOBNAME}_${SLURM_JOB_ID}_monthly.out"
            run_DQC_prod "${EXPVER}" "${MONTH_DATE}" "${MODEL,,}" "${DQC_PROFILE_PATH}" "${EXPERIMENT,,}" "${ACTIVITY,,}" \
                "${REALIZATION}" "${GENERATION}" "${N_PROC}" "${CHECK_STANDARD_COMPLIANCE}" \
                "${CHECK_SPATIAL_COMPLETENESS}" "${CHECK_SPATIAL_CONSISTENCY}" "${CHECK_PHYSICAL_PLAUSIBILITY}" \
                "${DQC_MONTHLY_OUTPUT_FILE}" "${MONTHLY_PROFILES}"
        else
            # Every model we run generates monthly profiles, so an empty result means generation broke.
            echo "ERROR: month-boundary split ${SPLIT_START_DATE} but no *monthly* profiles in ${DQC_PROFILE_PATH}." >&2
            exit 1
        fi
    fi
fi
...
# UPDATE_FDB_INFO defaults to enabled when the job does not define it, so
# plain (non-NRT) experiments keep updating the info file once per chunk.
if [ "${CREATE_FDB_INFO_FILE,,}" == "true" ] && [ "${UPDATE_FDB_INFO,,}" != "false" ]; then
    # Split jobs (NRT) advance the marker day by day; chunk jobs use the
    # chunk end date as before.
    DATA_END_DATE=${SPLIT_END_DATE:-${CHUNK_END_DATE}}
    ...
    singularity exec \
        ...
        --env "EXPVER=${EXPVER}" \
        --env "MODEL=${MODEL}" \
        --env "DATA_END_DATE=${DATA_END_DATE}" \
        ${bindings} \
        "${HPC_CONTAINER_DIR}"/gsv/gsv_${GSV_VERSION}.sif \
        bash -c \
        ' python3 ${SCRIPTDIR}/FDB/update_fdb_info.py \
		--file ${FDB_INFO_FILE_NAME} --data_end_date ${DATA_END_DATE} '
fi
