...
BRIDGE_EXPVER=${32:-%REQUEST.BRIDGE_EXPVER%}
...
export SINGULARITYENV_BRIDGE_EXPVER="${BRIDGE_EXPVER}"
    ...
    singularity exec "${HPC_CONTAINER_DIR}"/gsv/gsv_${GSV_VERSION}.sif \
        bash -c ' set -xuve
            if [ "${CHUNK}" == "1" ] && [ "${SPLIT_FIRST}" == "TRUE" ]; then
                ...
                --bridge_expver ${BRIDGE_EXPVER} --bridge_start_date ${START_DATE}
            else
                python3 ${SCRIPTDIR}/FDB/update_fdb_info.py --file ${FDB_INFO_FILE_NAME} \
                --bridge_expver ${BRIDGE_EXPVER} --bridge_end_date ${SPLIT_END_DATE}
            fi
        '
fi
