...
PERFORMANCE_PLATFORM_FLAGS=${93:-%CURRENT_PERFORMANCE_PLATFORM_FLAGS%}
RUNDIR_PATH=${94:-%CONFIGURATION.RUNDIR_PATH%}
AS_JOBNAME=${95:-%JOBNAME%}
...
RESOLUTION=${96:-%MODEL.RESOLUTION%}
CHUNK_START_DATE=${97:-%CHUNK_START_DATE%}
CHUNK_SECOND_TO_LAST_DATE=${98:-%CHUNK_SECOND_TO_LAST_DATE%}
DATASET=${99:-%REQUEST.DATASET%}
STREAM=${100:-%REQUEST.STREAM%}
COMPLEXITY_IFS=${101:-%PERFORMANCE_METRICS.COMPLEXITY.IFS%}
COMPLEXITY_NEMO=${102:-%PERFORMANCE_METRICS.COMPLEXITY.NEMO%}
PERFORMANCE_METRICS_VERSION=${103:-%PERFORMANCE_METRICS.VERSION%}
PERFORMANCE_RESOLUTION=${104:-%PERFORMANCE_METRICS.RESOLUTION%}
# NRT mode flag and split parameters (populated in NRT runs; False/empty in standard runs)
NRT=${105:-%CONFIGURATION.NRT%}
SPLIT_START_DATE=${106:-%SPLIT_START_DATE%}
SPLIT_END_DATE=${107:-%SPLIT_END_DATE%}
SPLIT=${108:-%SPLIT%}
SPLIT_FIRST=${109:-%SPLIT_FIRST%}
SPLIT_LAST=${110:-%SPLIT_LAST%}
...
# Standard: chunk-level dirs  (${PRE_RESTART_DIR}/${CHUNK}/)
# NRT:      split-level dirs  (${PRE_RESTART_DIR}/${CHUNK}/${SPLIT}/)
# In both cases RAPS reads/writes via the 'current' symlink.
#################################################

if [[ "${NRT,,}" == "true" ]]; then
    # NRT: one restart dir per split
    PRE_RESRART_SUBDIR="${PRE_RESTART_DIR}/${CHUNK}/${SPLIT}"
else
    # Standard: one restart dir per chunk
    PRE_RESRART_SUBDIR="${PRE_RESTART_DIR}/${CHUNK}"
fi

mkdir -p "${PRE_RESRART_SUBDIR}"
cd "${PRE_RESRART_SUBDIR}"
...
# Guard: a stale fesom_raw_restart at the run's *initial position* of a
# non-restarted run means this is a re-run — remove it so RAPS enters
# initial-conditions mode (no np64.info → reads initial fesom.clock files).
# The "initial position" is:
#   - standard : chunk 1                 (NRT != true)
#   - NRT      : chunk 1, split 1        (SPLIT == 1)
# hence the compound condition below. When RESTART_FROM is set the restarts were
# copied in by INI and must be preserved, so the guard does not fire.
if [[ "${CHUNK}" == "1" ]] &&
    { [[ "${NRT,,}" != "true" ]] || [[ "${SPLIT}" == "1" ]]; } && # initial position only
    [ -d "fesom_raw_restart" ] && [[ -z "${RESTART_FROM:-}" ]]; then
    echo "WARNING: Removing stale fesom_raw_restart from initial position (re-run detected)"
...
# Point the 'current' symlink (RAPS --restartdirectory) at the dir RAPS must
# read/write for this run. Single place that creates the symlink:
#   - standard     : this chunk's dir, refreshed every chunk.
#   - NRT split 1  : this split's dir, seeded by INI (chunk 1) or by the previous
#                    chunk's last-split restarts_moving (chunks 2+).
#   - NRT split 2…N: 'current' already points at the previous split's output —
#                    the restarts RAPS must read — so it is deliberately NOT
#                    repointed here; restarts_moving advances it after each split
#                    (that is the refresh the 2…N path still relies on).
cd "${PRE_RESTART_DIR}"
if [[ "${NRT,,}" != "true" ]]; then
    ln -sfn "${CHUNK}" current
elif [[ "${SPLIT}" == "1" ]]; then
    ln -sfn "${CHUNK}/${SPLIT}" current
fi
...
if [[ "${NRT,,}" == "true" ]]; then
    # NRT: size fclen to the end of this split (one day per split) by passing the
    # split end as the explicit run-period end.
    # lib/common/utils/sim_utils.sh (load_experiment_ifs) (auto generated comment)
    load_experiment_"${ATM_MODEL}" "${SIM_START_DATE}" "${SPLIT_END_DATE}"
else
    # lib/common/utils/sim_utils.sh (load_experiment_ifs) (auto generated comment)
    load_experiment_"${ATM_MODEL}" "${SIM_START_DATE}"
fi
...
echo "Moving restart files to the next split/chunk folder"
