...
HPC_CONTAINER_DIR=${15:-%CURRENT_CONTAINER_DIR%}
...
CHUNK_END_DATE=${25:-%CHUNK_END_DATE%}
...
. "${LIBDIR}"/common/utils/wipe_utils.sh
...
WIPE_REQUESTS_PATH=${CURRENT_ROOTDIR}/wipe_requests/${REALIZATION}
mkdir -p ${WIPE_REQUESTS_PATH}
cd ${WIPE_REQUESTS_PATH}

# Call the function and assign the result to WIPE_MONTHLY
# lib/common/util.sh (enable_process_monthly) (auto generated comment)
WIPE_MONTHLY=$(enable_process_monthly "$START_DATE" "$CHUNK_END_DATE")
...
# lib/common/utils/wipe_utils.sh (check_expver_match) (auto generated comment)
...
singularity exec --cleanenv --no-home \
    ...
    source "${LIBDIR}"/common/utils/wipe_utils.sh
    # Wipe clte data always
    # lib/common/utils/wipe_utils.sh (exec_wipe) (auto generated comment)
...
WIPE_DONE_DIR="${CURRENT_ROOTDIR}/wipe_done/${CHUNK}"
mkdir -p "${WIPE_DONE_DIR}"
touch "${WIPE_DONE_DIR}/r${REALIZATION}"

read -r -a MEMBERS_ARRAY <<<"${MEMBER_LIST}"
MEMBER_COUNT=${#MEMBERS_ARRAY[@]}
DONE_COUNT=$(find "${WIPE_DONE_DIR}" -maxdepth 1 -type f -name 'r*' | wc -l)

if [[ "${DONE_COUNT}" -ge "${MEMBER_COUNT}" ]]; then
    singularity exec \
        --env "SCRIPTDIR=${SCRIPTDIR}" \
