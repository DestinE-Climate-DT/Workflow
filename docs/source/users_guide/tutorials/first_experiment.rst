...
Before starting your first experiment, make sure you have the prerequisites and required configuration described in the :ref:`autosubmit`, :ref:`infrastructure`, and :ref:`prerequisites` sections of this documentation.
...
Basic instructions to execute the workflow with Autosubmit can be found in `How to run <https://earth.bsc.es/gitlab/digital-twins/de_340-2/workflow/-/wikis/How-to-run>`_. It is important to modify the minimal configuration file created for your experiment, located at ``/appl/AS/AUTOSUBMIT_DATA/<expid>/conf/minimal.yml``. In particular, update the `PROJECT_SUBMODULES` field as described in the linked page. If you do not want to use any submodules in your experiment, set this field to `False`. After making these changes, you can proceed to the next step.

Additional documentation is available in the same `Wiki <https://earth.bsc.es/gitlab/digital-twins/de_340-2/workflow/-/wikis/home>`_.
