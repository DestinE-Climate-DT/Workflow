...
SIM_START_DATE=${22:-%SDATE%}
NRT=${23:-%CONFIGURATION.NRT%}
...
. "${LIBDIR}"/common/utils/fix_constant_variables_utils.sh
. "${LIBDIR}"/"${HPC}"/config.sh

# lib/common/util.sh (get_member_number) (auto generated comment)
MEMBER_NUMBER=$(get_member_number "${MEMBER_LIST}" ${MEMBER})

# NRT: this job runs right after SIM split 2 (day 2 of chunk 1), so the
# chunk-end data is not in the FDB yet — retrieve the constant field from
# day 2 instead (it is constant in time; any simulated day works).
if [ "${NRT,,}" == "true" ]; then
...
singularity exec --cleanenv --no-home \
    ...
    cd ${HPCROOTDIR}
    . "${LIBDIR}"/common/utils/fix_constant_variables_utils.sh
    # lib/common/utils/fix_constant_variables_utils.sh (fix_constant_variables) (auto generated comment)
