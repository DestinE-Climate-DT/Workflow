...
- Near-real-time (NRT) Storylines prototype for IFS-FESOM. Each monthly chunk is split into
  daily Autosubmit splits so the SIM job advances one simulation day per split (28–31 splits
  per chunk), with restarts handled per split and only the chunk-boundary restart backed up.
  Two toggles control it:
  - `CONFIGURATION.NRT` — activate the NRT daily-split mode for SIM. Opt-in: off unless set.
  - `CONFIGURATION.DN` — in end-to-end runs, make the DN job depend on the matching SIM split
    so it flows daily instead of waiting for the last split of the chunk (default `True` for
    end-to-end).
- Jacamar-based CI for the `tsuite`
- NRT daily post-processing for IFS-FESOM: DQC, transfer, and constant-variable fixing now run
  per daily SIM split instead of once per chunk when their jobs are enabled.
- Added the capability of running the Model Serving Flow from DE_393 using the `simless` workflow by enabling the key `CONFIGURATION.MODEL_SERVING_FLOW: "True"`.
- Added a specific checker in LOCAL_SETUP to verify the flag COMPILE that stops the execution if the model compilation is not available.
- Added the section `users_guide/usage/prerequisites` in the documentation. It documents the access and permissions that new users may need to request before running the workflow.
- Minor documentation fixes.
- More unit tests for Local Setup.
...
- Energy indicators does run now monthly by default.
- Autosubmit version set to `4.1.16.1-bullseye-slim` for the CI (`.gitlab-ci.yml`)
- The user now has the ability to disable the compression functions in the CLEAN job by setting CONFIGURATION.TAR_RUNDIR and/or CONFIGURATION.TAR_LOGS to False.
- Updated the default AQUA container version to v0.19.13.
- Updated the AQUA docs to include recent changes.
- TDIGEST statistics for energy indicators are computed in a sigle OPA job now instead of 2 (TDIG instead of TDIG1 & TDIG2).
- Energy offshore was removed as it is not part of phase 3.
- The baseline tests now compare all generated files under the experiment's `tmp/` directory, not just the `.cmd` scripts of a fixed set of jobs.
...
- Changed nn_stock (restart writing step) in the NEMO standalone calculation from hours to steps, taking into account the NEMO timestep (rn_rdt).
- The data-portfolio cloning will no longer be checked if the model is set to NEMO.
...
- NRT is opt-in. Set `CONFIGURATION.NRT: "True"` in the experiment configuration to enable
  daily-split SIM behaviour; leave it unset (or `"False"`) for standard monthly chunks.
