...
setup() {
    ...
    export EXPVER="1234"
    export CHUNK_END_DATE="20220501"
    export MODEL="ifs-nemo"
    export SDATE="20220101"
    export CURRENT_ARCH="lumi"
    export JOBNAME="lra_generator_123456789"
...
@test "load_template" {
    run source "${ROOTDIR}/proj/${PROJDEST}/templates/aqua/lra_generator.sh" ${ROOTDIR} ${PROJDEST} ${EXPID} ${HPC_PROJECT} ${CONTAINER_VERSION} ${MODEL} ${EXPVER} ${CATALOG_NAME} ${CHUNK_END_DATE} ${SDATE} ${CURRENT_ARCH} ${JOBNAME} ${HPC_SCRATCH} ${EXPERIMENT_NAME}
    assert_success
}
