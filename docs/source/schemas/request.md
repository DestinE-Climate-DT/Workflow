...
| ACTIVITY | `string` | ✅ | `baseline` `projections` `story-nudging` |  |  | Activity key in the FDB |  |
| REALIZATION | `integer` | ✅ | integer |  |  | Realization key in the FDB |  |
| GENERATION | `integer` | ✅ | `1` `2` |  |  | Generation key in the FDB |  |
| EXPERIMENT | `string` | ✅ | `cont` `hist` `SSP1-2.6` `SSP3-7.0` `Tplus2.0K` `abrupt4xco2` |  |  | Experiment key in the FDB |  |
