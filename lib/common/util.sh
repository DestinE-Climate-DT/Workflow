...
function fix_constant_variables() {
    ...
    if [ ! -f flag_constant_variables_fixed_${REALIZATION} ]; then
        for PROFILE in ${DQC_PROFILE_PATH}/sfc_daily_*.yaml; do
            if [ -f "${PROFILE}" ]; then
                ...
                GRIB_FILE_NAME="$(basename "${PROFILE}" ".yaml")_chunk_${CHUNK}_real_${REALIZATION}.grb"
                ...
                grib_set -s date="${FIRST_DATE}" "${GRIB_FILE_NAME}" fixed_real_${REALIZATION}.grb

                # Write date-fixed file in FDB
                fdb-write fixed_real_${REALIZATION}.grb

                # Remove tempfiles
                rm "${GRIB_FILE_NAME}" fixed_real_${REALIZATION}.grb
            fi
        done
        # Create a flag to indicate that the constant variables have been fixed
        touch flag_constant_variables_fixed_${REALIZATION}
        echo "Constant variables fixed for realization ${REALIZATION}."
...
function check_expver_match() {
    ...
    if [[ "$transfer_modify__metadata_enabled" == "true" ]]; then
        ...
        if [[ "$has_expver_field" == "true" && -n "$bridge_expver" ]]; then
            ...
            echo "The transfer job is modifing the metadata, but the experimentversionnumber field is not present."
