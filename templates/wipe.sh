...
FDB_PROD=${14:-%CURRENT_FDB_PROD%}
SCRATCH_DIR=${15:-%CURRENT_SCRATCH_DIR%}
HPC_CONTAINER_DIR=${16:-%CONFIGURATION.CONTAINER_DIR%}
GSV_VERSION=${17:-%GSV.VERSION%}
LIBDIR=${18:-%CONFIGURATION.LIBDIR%}
SCRIPTDIR=${19:-%CONFIGURATION.SCRIPTDIR%}
WIPE_DOIT=${20:-%WIPE_DOIT%}
MEMBER=${21:-%MEMBER%}
MEMBER_LIST=${22:-%EXPERIMENT.MEMBERS%}
FDB_INFO_FILE_PATH=${23:-%REQUEST.INFO_FILE_PATH%}
FDB_INFO_FILE_NAME=${24:-%REQUEST.INFO_FILE_NAME%}
BASE_VERSION=${25:-%BASE.VERSION%}
CHUNK_END_DATE=${26:-%CHUNK_END_DATE%}
OPERATIONAL_PROJECT_SCRATCH=${27:-%CONFIGURATION.OPERATIONAL_PROJECT_SCRATCH%}
DEVELOPMENT_PROJECT_SCRATCH=${28:-%CONFIGURATION.DEVELOPMENT_PROJECT_SCRATCH%}
...
# Call the function to determine if the first day of the month is included in the chunk and assign the result to WIPE_MONTHLY
...
for profile_file in "${DQC_PROFILE_PATH}"/*.yaml; do
    ...
    if [[ "$profile_file" == *monthly* ]] && [[ "$WIPE_MONTHLY" == false ]]; then
        ...
        singularity exec --cleanenv --no-home \
            ...
            --bind "$(realpath ${OPERATIONAL_PROJECT_SCRATCH})" \
