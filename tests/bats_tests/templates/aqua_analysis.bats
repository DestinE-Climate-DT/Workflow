...
@test "load_template_without_mocked_pdfs" {
    run source "${ROOTDIR}/proj/${PROJDEST}/templates/aqua/aqua_analysis.sh" "${ROOTDIR}" "${PROJDEST}" "${EXPID}" "${HPC_PROJECT}" "${CONTAINER_VERSION}" "${MODEL}" "${EXPVER}" "${EXPERIMENT_NAME}" "${CATALOG_NAME}" "${APP_OUTPATH}" "${CURRENT_ARCH}" "${HPC_SCRATCH}"
    assert_failure
}
