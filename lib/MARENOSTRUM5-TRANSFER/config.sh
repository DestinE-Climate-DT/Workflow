    ...
    for app_name in $app_list; do
        ...
        ${CONTAINER_COMMAND} exec --cleanenv --no-home \
            ...
            python3 "${SCRIPTDIR}/FDB/preprocess_requests.py" \
                ...
                --output-dir="${base_dir}/requests" \
                --jobname=${jobname} \
                --process-derived-variables


            for request in ${base_dir}/requests/*${jobname}.yaml; do
                ...
                python3 "${SCRIPTDIR}/FDB/yaml_to_flat_request.py" \
                    --file="$request" --request_name="${base_dir}/requests/${FLAT_REQ_NAME}"
            done
            '

        for FLAT_REQ_NAME in ${base_dir}/requests/request_${app_name}_${jobname}_*.flat; do
