...
SPLIT_START_DATE=${44:-%SPLIT_START_DATE%}
...
HYDROMET_IN_DATA_VERSION=${49:-%HYDROMET.IN_DATA_VERSION%}
HYDROLAND_IN_DATA_VERSION=${50:-%HYDROLAND.IN_DATA_VERSION%}
WILDFIRES_WISE_IN_DATA_VERSION=${51:-%WILDFIRES_WISE.IN_DATA_VERSION%}
WILDFIRES_FWI_IN_DATA_VERSION=${52:-%WILDFIRES_FWI.IN_DATA_VERSION%}
APP_AUX_IN_DATA_DIR=${53:-%APP_AUX_IN_DATA_DIR%}
HPC_PROJECT_ROOT=${54:-%CURRENT_HPC_PROJECT_ROOT%}
SCRATCH_DIR=${55:-%CURRENT_SCRATCH_DIR%}
LOCAL_DIR=${56:-%CURRENT_LOCAL_DIR%}
HYDROLAND_STAT_FREQ=${57:-%HYDROLAND.STAT_FREQ%}
HYDROLAND_GRID=${58:-%HYDROLAND.GRID%}
HYDROLAND_TEMP=${59:-%HYDROLAND.TAVG_VARIABLE%}
HYDROLAND_PRE=${60:-%HYDROLAND.PRE_VARIABLE%}
HYDROLAND_RUN_FREQUENCY=${61:-%HYDROLAND.RUN_FREQUENCY%}
HYDROLAND_INIT_FILES=${62:-%HYDROLAND.INIT_FILES%}
HYDROLAND_BIAS_ADJUSTMENT=${63:-%HYDROLAND.BIAS_ADJUSTMENT.APPLY_BA%}
HYDROLAND_DELETE_FILES=${64:-%HYDROLAND.DELETE_FILES%}
HYDROLAND_APPLY_INDICATORS=${65:-%HYDROLAND.APPLY_INDICATORS%}
HYDROLAND_PRIOR_APP_OUTPATH=${66:-%HYDROLAND.PRIOR_APP_OUTPATH%}
HYDROLAND_RESTART_FROM_PRIOR_RUN=${67:-%HYDROLAND.RESTART_FROM_PRIOR_RUN%}
HYDROLAND_APPLY_CDO_MERGETIME=${68:-%HYDROLAND.APPLY_CDO_MERGETIME%}
OBSALL_VERSION=${69:-%OBSALL.VERSION%}
REQUEST_REALIZATION=${70:-%REQUEST.REALIZATION%}
HYDROMET_RUN_FREQUENCY=${71:-%HYDROMET.RUN_FREQUENCY%}
ENERGY_INDICATORS_RUN_FREQUENCY=${72:-%ENERGY_INDICATORS.RUN_FREQUENCY%}
...
OBSALL_IN_DATA_VERSION=${77:-%OBSALL.IN_DATA_VERSION%}
OPA_MP_PROC=${78:-%CURRENT_OPA_MP_PROC%}
DATA_PORTFOLIO=${79:-%CONFIGURATION.DATA_PORTFOLIO%}
...
function run_ENERGY_INDICATORS() {
    ...
    mkdir -p "${APP_OUTPATH}/${INI_YEAR}/${INI_MONTH}/"
    ...
    if [ "$ENERGY_INDICATORS_RUN_FREQUENCY" = "day" ] && [ -n "$SPLIT_INI_YEAR" ]; then
        # Script arguments
        runscript_args=(
        ...
        for iday in $(seq -w 1 $END_DAY); do
            # Script arguments
            runscript_args=(
                --iniyear "${INI_YEAR}"
                --inimonth "${INI_MONTH}"
                ...
                --finday "${iday}"
                --in_path "${OPA_OUTPATH}/"
                --out_path "${APP_OUTPATH}/${INI_YEAR}/${INI_MONTH}/"
                --mask_file "${MASK_FILE}"
                --mp_proc "${OPA_MP_PROC}"
            )
    ...
    if [ "${CHUNK_SECOND_TO_LAST_DATE}" = "${SPLIT_SECOND_TO_LAST_DATE}" ] || [ -z "${SPLIT_SECOND_TO_LAST_DATE}" ]; then
        ...
        WS_HISTO_FILE=${OPA_OUTPATHTDIG}/${INI_YEAR}_${INI_MONTH}*ws_timestep_60_monthly_histogram_bin*.nc
        WS_PERCENTILE_FILE=${OPA_OUTPATHTDIG}/${INI_YEAR}_${INI_MONTH}*ws_timestep_60_monthly_percentile.nc

        # other OPA files
        OPA_FILE=${OPA_OUTPATH}/${INI_YEAR}_${INI_MONTH}*ws_timestep_60_daily_max.nc

        if ls $WS_HISTO_FILE 1>/dev/null 2>&1; then
            mv $WS_HISTO_FILE ${APP_OUTPATH}/${INI_YEAR}/${INI_MONTH}/
            echo "WS histogram file(s) copied successfully."
        else
            echo "No WS histogram file found."
        fi
        if ls $WS_PERCENTILE_FILE 1>/dev/null 2>&1; then
            mv $WS_PERCENTILE_FILE ${APP_OUTPATH}/${INI_YEAR}/${INI_MONTH}/
            echo "WS 99th percentile file(s) copied successfully."
        else
            echo "No 99th percentile file found."
        fi
        if ls $OPA_FILE 1>/dev/null 2>&1; then
            mv $OPA_FILE ${APP_OUTPATH}/${INI_YEAR}/${INI_MONTH}/
        ...
        dir="${APP_OUTPATH}/${INI_YEAR}/${INI_MONTH}/"
...
if [[ "${APP^^}" == "ENERGY_INDICATORS" ]]; then
    export OPA_OUTPATHTDIG="${HPCROOTDIR}/opa/energytdig/${DATELIST}/${MEMBER_NAME}/"
fi
