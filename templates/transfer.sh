...
if [ "${NRT,,}" == "true" ]; then
    # NRT transfers run per daily split, before the month is fully simulated.
    # Monthly streams only reach the FDB once the month completes, so they are
    # transferred by the split that crosses the month boundary.
    # lib/common/util.sh (requires_process_monthly_nrt) (auto generated comment)
    TRANSFER_MONTHLY=$(requires_process_monthly_nrt "$START_DATE" "$SPLIT_END_DATE")
else
