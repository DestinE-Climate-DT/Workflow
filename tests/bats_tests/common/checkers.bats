...
setup() {
    ...
    # checker_members uses get_realization_list
...
@test "checker_submodules all errors" {
    ...
    expected_output='The data-portfolio submodule for the workflow has failed to clone. The ifs-nemo submodule has failed to clone. The data-portfolio submodule for AQUA has failed to clone. The DVC cache submodule has failed to clone.'
...
@test "checker_catalog fails when catalog directory is missing" {
    run checker_catalog
    assert_failure
    assert_output "The Climate-DT catalog has failed to clone."
}

@test "checker_catalog fails when catalog directory is empty" {
    mkdir -p "${ROOTDIR}/tmp/catalog"
    run checker_catalog
    assert_failure
    assert_output "The Climate-DT catalog has failed to clone."
}

@test "checker_catalog succeeds when catalog is populated" {
    mkdir -p "${ROOTDIR}/tmp/catalog"
    touch "${ROOTDIR}/tmp/catalog/entry.yaml"
    run checker_catalog
    assert_success
    refute_output
}
...
@test "checker_members accepts a list with one realization per member" {
    run checker_members "fc0 fc1 fc2"
    assert_success

    run checker_members "fc10 fc11"
    assert_success
}

@test "checker_members rejects a repeated member" {
    run checker_members "fc0 fc0"
    assert_failure
    assert_line --partial 'duplicated values in EXPERIMENT.MEMBERS'
}

@test "checker_members rejects zero-padded aliases of the same realization" {
    # fc8 and fc08 are distinct members but both resolve to realization 9
    run checker_members "fc8 fc08"
    assert_failure
    assert_line --partial 'duplicated values in EXPERIMENT.MEMBERS'
}

@test "checker_members rejects a member outside the fcN format" {
    run checker_members "default"
    assert_failure
}

@test "checker_members rejects an empty member list" {
    run checker_members ""
    assert_failure
    assert_line --partial 'EXPERIMENT.MEMBERS is not defined'
}
