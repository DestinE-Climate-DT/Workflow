...
LOG_DIR=${28:-%CONFIGURATION.HPC_LOG_DIR%}
...
# Find rundirs matching the SIM job name and chunk start date, then compress and archive them.
find_results=$(find "${HPCROOTDIR}" -type d \( -name "h*${SIM_JOBNAME}*" -o -name "run_${CHUNK_START_DATE}*${SIM_JOBNAME}*" \) -print)
while IFS= read -r rundir; do
    if [ -n "${rundir}" ]; then
        tar --exclude='*.tar.gz' -I 'pigz -p ${SLURM_CPUS_PER_TASK:-1}' -cf "${rundir}_${timestamp}.tar.gz" "${rundir}" && rm -rf "${rundir}"
    fi
done <<<"$find_results"

# Compresses and archives SIM job log files.
tar --exclude='*.tar.gz' -I 'pigz -p ${SLURM_CPUS_PER_TASK:-1}' -cf "${LOG_DIR}/${SIM_JOBNAME}_${timestamp}.tar.gz" "${LOG_DIR}/${SIM_JOBNAME}"*
