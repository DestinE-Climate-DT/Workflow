...
def walk_callback(
    ...
    for f in files:
        ...
        # An empty filter means "no filter": compare every generated file.
        if extensions and file_abs_path.extension() not in extensions:
            continue
        if expected_scripts and not any(e in f for e in expected_scripts):
            continue
        if file_parent_dir == ".":
