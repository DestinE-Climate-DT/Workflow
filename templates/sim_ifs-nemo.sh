...
CHUNK_START_DATE=${16:-%CHUNK_START_DATE%}
...
HPC_PROJECT_ROOT=${63:-%CURRENT_HPC_PROJECT_ROOT%}
LOCAL_DIR=${64:-%CURRENT_LOCAL_DIR%}
SIM_START_DATE=${65:-%SDATE%}
RUNDIR_PATH=${66:-%CONFIGURATION.RUNDIR_PATH%}
...
PRE_RESTART_DIR=${HPCROOTDIR}/restarts/${SIM_START_DATE}/${MEMBER}
...
function run_experiment_ifs() {
    ...
    # errors in the hres are caught
    set -o pipefail

    # if there are failures in hres the command fails, but
    # it doesn't fail if the "grep" returns nothing
    hres \
        -p "$mpi" -t "$omp" -h "$ht" \
        -j "$jobid" -J "$jobname" \
        ...
        grep -v 'IO_SERV_CLOSE_EC FLUSHFDB' || true
}
