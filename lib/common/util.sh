...
function get_member_number() {
    ...
    elif [[ "$MEMBER" == "default" ]]; then
        # legacy ICON member name; runs created before v6.2.4 wrote realization 1
        echo "1"
    else
        echo "Member '${MEMBER}' is not in the expected fcN format or different than default." >&2
        exit 1
    fi
}
