...
# Arguments:
#   $1  sim_start_date  simulation start date. Its offset from the fixed IFS
#       restart base date (IFS_START_DATE) is added to every fclen, so restarts
#       from another simulation size correctly. Defaults to SIM_START_DATE.
#   $2  this_chunk_end_date  optional explicit end of the current run period
#       (e.g. SPLIT_END_DATE for an NRT day split). When given, fclen is sized to
#       this date instead of to the chunk size. Empty for a standard chunk run.
function load_experiment_ifs() {

    local sim_start_date=${1:-${SIM_START_DATE}}
    local this_chunk_end_date=${2:-}
    ...
    # Offset between the IFS restart's fixed base date and the simulation start
    # (non-zero only when restarting from another simulation).
    t1=$(TZ=UTC date -d "${IFS_START_DATE}" +%s)
    t2=$(TZ=UTC date -d "${sim_start_date}" +%s)
    offset_days=$(((t2 - t1) / 86400))

    if [ -n "${this_chunk_end_date}" ]; then
        # Explicit run-period end (e.g. NRT day-split SPLIT_END_DATE): size fclen to
        # this period's length plus the restart offset, i.e. days from IFS_START_DATE.
        t3=$(TZ=UTC date -d "${this_chunk_end_date}" +%s)
        runlength=$(((t3 - t2) / 86400 + offset_days))
        CHUNKSIZEUNIT=day
    elif [ "${CHUNKSIZEUNIT,,}" == "month" ] || [ "${CHUNKSIZEUNIT,,}" == "year" ]; then
        runlength=$((CHUNK_END_IN_DAYS + offset_days))
        CHUNKSIZEUNIT=day
    else
        # For daily chunks
        runlength=$((CHUNK * CHUNKSIZE + offset_days))
    fi
...
# and moving them to the next folder so the following
# run finds them. Standard runs move chunk N -> N+1;
# NRT day-split runs (NRT=true) move along the 'current'
# symlink to the next split, or to the next chunk's first
# split on the last split of a chunk.
################################################
# lib/common/utils/sim_utils.sh (restarts_moving) (auto generated comment)
function restarts_moving() {
    local nrt_flag="${NRT:-false}"
    local is_nrt=false
    [[ "${nrt_flag,,}" == "true" ]] && is_nrt=true

    cd "${PRE_RESTART_DIR}" || return 1

    # Common IFS restart filename components (shared by both models).
    local sdate_long="${IFS_START_DATE}000000"
    local restart_day
    printf -v restart_day "%06d" "$((runlength - 1))"

    # Mode-specific source/destination dirs and the srf filename stamp.
    # Standard runs move chunk N -> N+1. NRT day-splits move along the 'current'
    # symlink (RAPS reads AND writes ${PRE_RESTART_DIR}/current) to the next
    # split's dir, or to (CHUNK+1)/1 on the last split of a chunk.
    local src_dir dst_dir srf_stamp
    if [[ "${is_nrt}" == "true" ]]; then
        src_dir="current"
        printf -v srf_stamp "%06d" "${runlength}"
        if [[ "${SPLIT_LAST}" == "TRUE" ]]; then
            dst_dir="$((CHUNK + 1))/1"
        else
            dst_dir="${CHUNK}/${SPLIT}"
        fi
        ...
        src_dir="${CHUNK}"
        dst_dir="$((CHUNK + 1))"
        printf -v srf_stamp "%06d0000" "${runlength}"
    fi

    mkdir -p -- "${dst_dir}"

    local src_abs dst_abs
    src_abs=$(readlink -f -- "${src_dir}") || return 1
    dst_abs=$(readlink -f -- "${dst_dir}") || return 1

    # On NRT split 1 the template already pointed current at CHUNK/SPLIT, so
    # source and destination resolve to the same dir and the moves are skipped
    # (idempotent). For standard chunks src and dst always differ.
    if [[ "${src_abs}" != "${dst_abs}" ]]; then
        # NRT restores the input-restart backups BEFORE the move (the backup
        # advances with the split); standard restores them AFTER (see below) so a
        # retried chunk re-reads its own input.
        if [[ "${is_nrt}" == "true" ]]; then
            [[ -f "${src_dir}/rcf-backup" ]] && mv -- "${src_dir}/rcf-backup" "${src_dir}/rcf"
            [[ -f "${src_dir}/waminfo-backup" ]] && mv -- "${src_dir}/waminfo-backup" "${src_dir}/waminfo"
        fi

        # Common IFS atmospheric restart files.
        mv -- "${src_dir}/LAW${sdate_long}_${restart_day}"* "${dst_dir}/"
        mv -- "${src_dir}/srf${srf_stamp}"* "${dst_dir}/"
        mv -- "${src_dir}/BLS${sdate_long}_${restart_day}"* "${dst_dir}/"
        mv -- "${src_dir}/waminfo" "${dst_dir}/"
        mv -- "${src_dir}/rcf" "${dst_dir}/"

        # Model-specific ocean restart files. NEMO is not used in NRT, so it is
        # handled in the standard path only and left exactly as before (literal
        # PRE_RESTART_DIR sed pattern, original target).
        case "${MODEL_NAME,,}" in
        ifs-nemo)
            mv -- "${src_dir}/${EXPVER}_${END_DATE}"* "${dst_dir}/"
            mv -- "${src_dir}/nemorcf" "${dst_dir}/"
            mv -- "${src_dir}/nemorcf.${END_DATE}"* "${dst_dir}/"
            sed -i "s#${PRE_RESTART_DIR}/${CHUNK}#${PRE_RESTART_DIR}/$((CHUNK + 1))#" ${RESTART_DIR}/../$((CHUNK + 1))/nemorcf

            # Restore NEMO backups for retrials (after the move).
            [[ -f "${src_dir}/nemorcf-backup" ]] && mv -- "${src_dir}/nemorcf-backup" "${src_dir}/nemorcf"
            [[ -f "${src_dir}/nemorcf.${CHUNK_START_DATE}-backup" ]] && mv -- "${src_dir}/nemorcf.${CHUNK_START_DATE}-backup" "${src_dir}/nemorcf.${CHUNK_START_DATE}_000000"
            ;;
        ifs-fesom)
            if [[ "${is_nrt}" == "true" ]]; then
                if [[ -d "${src_dir}/fesom_raw_restart" ]]; then
                    # Restore FESOM backup before moving (advances with the split).
                    [[ -d "${src_dir}/fesom_raw_restart-backup" ]] && mv -- "${src_dir}/fesom_raw_restart-backup" "${src_dir}/fesom_raw_restart"
                    [[ -d "${dst_dir}/fesom_raw_restart" ]] && rm -rf -- "${dst_dir}/fesom_raw_restart"
                    mv -- "${src_dir}/fesom_raw_restart" "${dst_dir}/"
                fi
            else
                # Remove stale destination from a prior retrial, then move.
                [[ -d "${dst_dir}/fesom_raw_restart" ]] && rm -rf -- "${dst_dir}/fesom_raw_restart"
                mv -- "${src_dir}/fesom_raw_restart" "${dst_dir}/"
                # Restore FESOM backup into the source chunk for retrials (after).
                [[ -d "${src_dir}/fesom_raw_restart-backup" ]] && mv -- "${src_dir}/fesom_raw_restart-backup" "${src_dir}/fesom_raw_restart"
            fi
            ;;
        esac

        # Standard restores the IFS input-restart backups after the move.
        if [[ "${is_nrt}" != "true" ]]; then
            [[ -f "${src_dir}/rcf-backup" ]] && mv -- "${src_dir}/rcf-backup" "${src_dir}/rcf"
            [[ -f "${src_dir}/waminfo-backup" ]] && mv -- "${src_dir}/waminfo-backup" "${src_dir}/waminfo"
        fi
    ...
    # Update the 'current' symlink so the next run reads from the right dir.
    if [[ "${is_nrt}" == "true" ]]; then
        ln -sfn "${dst_dir}" current
    else
        rm -rf -- current
