...
Welcome to the Climate DT Workflow! The documentation can be accessed in [ReadTheDocs](https://climate-dt-workflow.readthedocs.io/en/stable/)

## Getting Started

All the experiments should be created in the [Autosubmit Virtual Machine](https://wiki.eduuni.fi/spaces/cscRDIcollaboration/pages/462100843/Autosubmit+Virtual+Machine). To access the VM, you need a user and your SSH key to be authorized. Add your name, e-mail, preferred username, and SSH (local) public key to the [table](https://wiki.eduuni.fi/spaces/cscRDIcollaboration/pages/462100841/Autosubmit+Virtual+Machine+Users?src=contextnavpagetreemode).

To access the [GUI](https://climatedt-wf.csc.fi/), you need to be part of the [DestinE-Climate-DT](https://github.com/DestinE-Climate-DT) organization, and add your GitHub username in this [table](https://wiki.eduuni.fi/spaces/cscRDIcollaboration/pages/502869547/Autosubmit+GUI+users). This is not mandatory but highly reccomended.

Make sure you have a recent Autosubmit version running `autosubmit --version`. Check by doing `module spider autosubmit` and then selecting the latest version of Autosubmit. The version of Autosubmit that has been used for each release can also be found in the [CHANGELOG.md](CHANGELOG.md). You can follow more detailed description about Autosubmit in [Autosubmit Readthedocs](https://autosubmit.readthedocs.io/en/master/). For issues related to Autosubmit, open an issue in the [Autosubmit repository](https://github.com/BSC-ES/autosubmit/issues).

### Prerequisites

Inside the Autosubmit VM, you need to put your user configurations for platforms in `~/platforms.yml`:
...
experiments. Further instructions can be found [here](https://wiki.eduuni.fi/spaces/cscRDIcollaboration/pages/462100843/Autosubmit+Virtual+Machine) (Section 4. How to get password-less access from VM to LUMI / MN5).
...
Basic instructions to execute the workflow with Autosubmit can be found in [How to run](https://earth.bsc.es/gitlab/digital-twins/de_340-2/workflow/-/wikis/How-to-run).
...
A PDF of the structure of the workflow will pop up. The experiment will also be visible [in the GUI](https://climatedt-wf.csc.fi/). Close the PDF to continue.
...
The documentation (latest release) is available in [ReadTheDocs](https://climate-dt-workflow.readthedocs.io/en/stable/).

To build the documentation locally, you must clone the repo (`git clone https://earth.bsc.es/gitlab/digital-twins/de_340-2/workflow.git`) and:
...
2. Set up your environment like explained in [Development environment](https://earth.bsc.es/gitlab/digital-twins/de_340-2/workflow/-/blob/main/docs/source/developers_guide/development_environment.rst).
...
- Aina Gaya (Activity leader, IFS-NEMO Workflow developer): **aina.gayayavila[@]bsc.es**
- Leo Arriola (ICON Workflow developer): **leo.arriola[@]bsc.es**
- Muhammad Shafeeque (IFS-FESOM Workflow developer): **muhammad.shafeeque[@]awi.de**
- Miguel Andrés-Martínez (IFS-FESOM Workflow developer): **miguel.andres-martinez[@]awi.de**
...
- Miguel Castrillo (Former activity leader): **miguel.castrillo[@]bsc.es**
- Carmen Piñero (Workflow developer): **carmen.pinero[@]bsc.es**
- Marc Romeu (Workflow developer): **marc.romeu[@]bsc.es**
...
The license of the workflow project can be found in the LICENCE file.
...
A full list of all the software used, either through CI/CD, containers, can be found in the file software_disclaimer.txt
