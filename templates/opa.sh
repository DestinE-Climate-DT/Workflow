...
OPA_MP_PROC=${29:-%CURRENT_OPA_MP_PROC%}
OPA_LOG_LEVEL=${30:-%OPA_LOG_LEVEL%}
ENERGY_INDICATORS_IN_DATA_VERSION=${31:-%ENERGY_INDICATORS.IN_DATA_VERSION%}
ENERGY_OFFSHORE_IN_DATA_VERSION=${32:-%ENERGY_OFFSHORE.IN_DATA_VERSION%}
HYDROMET_IN_DATA_VERSION=${33:-%HYDROMET.IN_DATA_VERSION%}
HYDROLAND_IN_DATA_VERSION=${34:-%HYDROLAND.IN_DATA_VERSION%}
WILDFIRES_WISE_IN_DATA_VERSION=${35:-%WILDFIRES_WISE.IN_DATA_VERSION%}
WILDFIRES_FWI_IN_DATA_VERSION=${36:-%WILDFIRES_FWI.IN_DATA_VERSION%}
OBSALL_IN_DATA_VERSION=${37:-%OBSALL.IN_DATA_VERSION%}
DATA_IN_DATA_VERSION=${38:-%DATA.IN_DATA_VERSION%}
APP_AUX_IN_DATA_DIR=${39:-%APP_AUX_IN_DATA_DIR%}
OPERATIONAL_PROJECT_SCRATCH=${40:-%CONFIGURATION.OPERATIONAL_PROJECT_SCRATCH%}
DEVELOPMENT_PROJECT_SCRATCH=${41:-%CONFIGURATION.DEVELOPMENT_PROJECT_SCRATCH%}
HPC_PROJECT_ROOT=${42:-%CURRENT_HPC_PROJECT_ROOT%}
LOCAL_DIR=${43:-%CURRENT_LOCAL_DIR%}
MEMBER=${44:-%MEMBER%}
REQUEST_REALIZATION=${45:-%REQUEST.REALIZATION%}
CHUNK_START_MONTH=${46:-%CHUNK_START_MONTH%}
SPLIT_SECOND_TO_LAST_DATE=${47:-%SPLIT_SECOND_TO_LAST_DATE%}
ENERGY_INDICATORS_MASK_FILE_5=${48:-%ENERGY_INDICATORS.MASK_FILE_5%}
ENERGY_INDICATORS_MASK_FILE_10=${49:-%ENERGY_INDICATORS.MASK_FILE_10%}
ENERGY_INDICATORS_MASK_FILE_25=${50:-%ENERGY_INDICATORS.MASK_FILE_25%}
ENERGY_INDICATORS_GRID=${51:-%ENERGY_INDICATORS.GRID%}
...
APP_OUTPATH="${HPCROOTDIR}/output/${APP_NAME}/${DATELIST}/${MEMBER_NAME}/"
OPA_OUTPATH="${HPCROOTDIR}/opa/${APP_NAME}/${DATELIST}/${MEMBER_NAME}/"
...
if [ "${IS_END_OF_CHUNK}" -eq 1 ] || [ "${IS_END_OF_YEAR}" -eq 1 ]; then
    OPA_CHECKPOINT="${HPCROOTDIR}/checkpoint/00/${APP_NAME}/${DATELIST}/${MEMBER_NAME}/"
...
for i in {1..3}; do
    checkpoint_dir=("${HPCROOTDIR}/checkpoint/$(printf "%02d" "$i")/${APP_NAME}/${DATELIST}/${MEMBER_NAME}/")
...
for i in 12 24; do
    checkpoint_dir=("${HPCROOTDIR}/checkpoint/${i}/${APP_NAME}/${DATELIST}/${MEMBER_NAME}/")
    if [ ! -d "${checkpoint_dir}" ]; then
        mkdir -p "${checkpoint_dir}"
    fi
...
if [[ "${APP_NAME^^}" == *"ENERGYTDIG"* ]]; then
    OPA_APP_BA_AUX="${APP_AUX_IN_DATA_DIR}/energy_indicators_${DATA_VERSION}"
    OPA_APP_MASK_FILE="${APP_AUX_IN_DATA_DIR}/energy_indicators_v${ENERGY_INDICATORS_IN_DATA_VERSION}/${ENERGY_INDICATORS_MASK_FILE}"
else
    OPA_APP_BA_AUX="${APP_AUX_IN_DATA_DIR}/${APP_NAME}_${DATA_VERSION}"
    OPA_APP_MASK_FILE="None"
fi
...
# Export OPA variables for use by run_OPA function
export OPA_MAX_PROC
export OPA_LOG_LEVEL
export OPA_REQUEST_DIR=${LOGDIR}
export OPA_OUTPATH
export OPA_APP_BA_AUX
export OPA_APP_MASK_FILE
export CHUNK
export SPLIT
export EXPID
export APP_NAME
...
export PROJDEST
export SCRIPTDIR
export APP_AUX_IN_DATA_DIR
export DEVELOPMENT_PROJECT_SCRATCH
export OPERATIONAL_PROJECT_SCRATCH
export HPC_CONTAINER_DIR
export READ_FROM_DATABRIDGE
export GSV_WEIGHTS_PATH
export LOGDIR
export OPA_RESTART_DIR="${OPA_CHECKPOINT}"
export OPA_MP_PROC
...
for i in $(seq 1 "$NUM_REQUESTS"); do
    echo "Launching OPA request ${i} of ${NUM_REQUESTS}"
