...
SPLIT_START_DATE=${45:-%SPLIT_START_DATE%}
SPLIT_SECOND_TO_LAST_DATE=${46:-%SPLIT_SECOND_TO_LAST_DATE%}
PROJECT=${47:-%CURRENT_PROJECT%}
SPLITS=${48:-%SPLITS%}
ENERGY_INDICATORS_IN_DATA_VERSION=${49:-%ENERGY_INDICATORS.IN_DATA_VERSION%}
...
HPC_PROJECT_ROOT=${56:-%CURRENT_HPC_PROJECT_ROOT%}
SCRATCH_DIR=${57:-%CURRENT_SCRATCH_DIR%}
LOCAL_DIR=${58:-%CURRENT_LOCAL_DIR%}
HYDROLAND_STAT_FREQ=${59:-%HYDROLAND.STAT_FREQ%}
HYDROLAND_GRID=${60:-%HYDROLAND.GRID%}
HYDROLAND_TEMP=${61:-%HYDROLAND.TAVG_VARIABLE%}
HYDROLAND_PRE=${62:-%HYDROLAND.PRE_VARIABLE%}
HYDROLAND_RUN_FREQUENCY=${63:-%HYDROLAND.RUN_FREQUENCY%}
HYDROLAND_INIT_FILES=${64:-%HYDROLAND.INIT_FILES%}
HYDROLAND_BIAS_ADJUSTMENT=${65:-%HYDROLAND.BIAS_ADJUSTMENT.APPLY_BA%}
HYDROLAND_DELETE_FILES=${66:-%HYDROLAND.DELETE_FILES%}
HYDROLAND_APPLY_INDICATORS=${67:-%HYDROLAND.APPLY_INDICATORS%}
HYDROLAND_PRIOR_APP_OUTPATH=${68:-%HYDROLAND.PRIOR_APP_OUTPATH%}
HYDROLAND_RESTART_FROM_PRIOR_RUN=${69:-%HYDROLAND.RESTART_FROM_PRIOR_RUN%}
HYDROLAND_OMP_NUM_THREADS_MHM=${70:-%HYDROLAND.OMP_NUM_THREADS_MHM%}
HYDROLAND_SUBDOMAIN=${71:-%HYDROLAND.SUBDOMAIN%}
HYDROLAND_STAGE=${72:-%HYDROLAND.STAGE%}
HYDROLAND_NUM_SUBDOMAINS=${73:-%HYDROLAND.NUM_SUBDOMAINS%}
HYDROLAND_MAX_WORKERS=${74:-%HYDROLAND.MAX_WORKERS%}
HYDROLAND_SUBDOMAINS_PER_NODE=${75:-%HYDROLAND.SUBDOMAINS_PER_NODE%}
HYDROLAND_MEM_PER_SUBDOMAIN=${76:-%HYDROLAND.MEM_PER_SUBDOMAIN%}
HYDROLAND_MERGE_BUFFER_TARGET_MB=${77:-%HYDROLAND.MERGE_BUFFER_TARGET_MB%}
HYDROLAND_MERGE_SPLIT_NODES=${78:-%HYDROLAND.MERGE_SPLIT_NODES%}
HYDROLAND_APPLY_CDO_MERGETIME=${79:-%HYDROLAND.APPLY_CDO_MERGETIME%}
HYDROLAND_MRM_PARALLEL_SUBDOMAINS=${80:-%HYDROLAND.MRM_PARALLEL_SUBDOMAINS%}
HYDROLAND_MERGE_COMPRESSION_LEVEL=${81:-%HYDROLAND.MERGE_COMPRESSION_LEVEL%}
OBSALL_VERSION=${82:-%OBSALL.VERSION%}
REQUEST_REALIZATION=${83:-%REQUEST.REALIZATION%}
HYDROMET_RUN_FREQUENCY=${84:-%HYDROMET.RUN_FREQUENCY%}
ENERGY_INDICATORS_MASK_FILE_5=${85:-%ENERGY_INDICATORS.MASK_FILE_5%}
ENERGY_INDICATORS_MASK_FILE_10=${86:-%ENERGY_INDICATORS.MASK_FILE_10%}
ENERGY_INDICATORS_MASK_FILE_25=${87:-%ENERGY_INDICATORS.MASK_FILE_25%}
ENERGY_INDICATORS_GRID=${88:-%ENERGY_INDICATORS.GRID%}
ENERGY_OFFSHORE_COMPUTE_ICING=${89:-%ENERGY_OFFSHORE.COMPUTE_ICING%}
OBSALL_IN_DATA_VERSION=${90:-%OBSALL.IN_DATA_VERSION%}
OPA_MP_PROC=${91:-%CURRENT_OPA_MP_PROC%}
DATA_PORTFOLIO=${92:-%CONFIGURATION.DATA_PORTFOLIO%}
...
function run_ENERGY_INDICATORS() {
    ...
    env_args=(
        ...
        --env "OPA_MP_PROC=${OPA_MP_PROC}"
    )

    # Script arguments
    runscript_args=(
        --iniyear "${SPLIT_INI_YEAR}"
        --inimonth "${SPLIT_INI_MONTH}"
        --iniday "${SPLIT_INI_DAY}"
        --finyear "${SPLIT_INI_YEAR}"
        --finmonth "${SPLIT_INI_MONTH}"
        --finday "${SPLIT_INI_DAY}"
        --in_path "${OPA_OUTPATH}/"
        --out_path "${APP_OUTPATH}/${SPLIT_INI_YEAR}/${SPLIT_INI_MONTH}/"
        --mask_file "${MASK_FILE}"
        --mp_proc "${OPA_MP_PROC}"
    )

    # Container
    container="${HPC_CONTAINER_DIR}/energy_indicators/energy_indicators_${ENERGY_INDICATORS_VERSION}.sif"
    ...
    base_cmd=(
        ...
        "${container}"
        python3
        "${SCRIPTDIR}/energy_indicators/run_energy_indicators.py"
        "${runscript_args[@]}"
    )

    # Execute first stage of script
    "${base_cmd[@]}" --run_type 11 &
    "${base_cmd[@]}" --run_type 12 &
    "${base_cmd[@]}" --run_type 13 &
    "${base_cmd[@]}" --run_type 14 &
    wait

    # Execute second stage of script
    "${base_cmd[@]}" --run_type 2

    # Execute third stage of script
    "${base_cmd[@]}" --run_type 31 &
    "${base_cmd[@]}" --run_type 32 &
    "${base_cmd[@]}" --run_type 33 &
    "${base_cmd[@]}" --run_type 34 &
    wait
...
function run_ENERGY_OFFSHORE() {
    ...
    # TODO: Safely remove variable ENERGY_OFFSHORE.COMPUTE_ICING and rely solely on DATA_PORTFOLIO
    # TODO: Move this logic to runscript level
    icing_flag="${ENERGY_OFFSHORE_COMPUTE_ICING:-false}"
    icing_flag="${icing_flag,,}"

    if [ "$icing_flag" = "true" ] || [ "$DATA_PORTFOLIO" = "full" ]; then
        ENERGY_OFFSHORE_COMPUTE_ICING=true
    else
        ENERGY_OFFSHORE_COMPUTE_ICING=false
    fi
...
# run HYDROLAND
# GLOBALS:
#       LIBDIR, HYDROLAND_RUN_FREQUENCY
#####################
function run_HYDROLAND() {
    source "${LIBDIR}"/application/hydroland.sh
    run_HYDROLAND
}
...
function run_OBSALL() {
    ...
    # Set datatype(s) based on portfolio
    if [ "$DATA_PORTFOLIO" == "full" ]; then
        datatype=("SYNOP" "TEMP")
    elif [ "$DATA_PORTFOLIO" == "reduced" ]; then
        datatype=("SYNOP")
    else
        echo "Warning: Unknown portfolio '$DATA_PORTFOLIO'. Defaulting to datatype SYNOP and TEMP." >&2
        datatype=("SYNOP" "TEMP")
    fi
    ...
    # Remove GSV files that have been used. -_> this will be done at WF level
    rm ${OPA_OUTPATH}/${SPLIT_INI_YEAR}_${SPLIT_INI_MONTH}_${SPLIT_INI_DAY}_*raw_data.nc
}
...
# Zero-padded to a minimum of two digits: realization 1 -> member01, 100 -> member100
MEMBER_NAME="member$(printf '%02d' "$REALIZATION")"

# APP and OPA outpaths
APP_OUTPATH="${HPCROOTDIR}/output/${APP,,}/${DATELIST}/${MEMBER_NAME}/"
OPA_OUTPATH="${HPCROOTDIR}/opa/${APP,,}/${DATELIST}/${MEMBER_NAME}/"
# trick to deal with multiple opas for energy indicators.
if [[ "${APP^^}" == "ENERGY_INDICATORS" ]]; then
    export OPA_OUTPATHTDIG1="${HPCROOTDIR}/opa/energytdig1/${DATELIST}/${MEMBER_NAME}/"
    export OPA_OUTPATHTDIG2="${HPCROOTDIR}/opa/energytdig2/${DATELIST}/${MEMBER_NAME}/"
fi
