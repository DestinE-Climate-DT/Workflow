...
def main():
    ...
    for file_path in chain(Path(start_dir).rglob('*.[yY][mM][lL]'), Path(start_dir).rglob('*.[yY][aA][mM][lL]')):
        with open(file_path, 'r') as request:
            oparequest: Dict[str, Union[str, Dict]] = yaml.load(request, Loader=yaml.FullLoader)
            logging.debug('----------------')
    ...
    output_dict = {
        k: dict(v)
        for k, v in zip(range(1, len(allvariables) + 1), allvariables)
    }
    mother_request = {
        'GSVREQUEST': oparequest['GSVREQUEST'],  # Using the last GSV loaded as we expect them to be the same for now.
