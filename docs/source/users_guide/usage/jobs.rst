...
- Compute realization from the member identifier: ``fcN`` gives realization ``N+1`` (``MEMBERS: fc10`` resolves to realization 11). A member list can start with any member and does not have to start with fc0.
...
- Compute realization from the member identifier: ``fcN`` gives realization ``N+1`` (``MEMBERS: fc10`` resolves to realization 11). A member list can start with any member and does not have to start with fc0.
- If ensemble perturbation is enabled (``FESOM_PERTURB: "true"``), appends ``--fesom-perturb`` and ``--fesom-perturbation-seed`` flags with a member-dependent seed: ``FESOM_PERTURBATION_SEED_BASE`` + realization (``fc0`` → 50001, ``fc10`` → 50011). The seed therefore follows the member name, not its position in the list.
...
Under NRT runs, DQC runs once per daily SIM split instead of once per chunk, via the ``conf/toggles/dqc-True-nrt-True.yml`` override. See :ref:`NRT runs <nrt>`.
...
Under NRT runs, the transfer runs once per daily SIM split instead of once per chunk, via the ``conf/toggles/transfer-True-nrt-True.yml`` override; monthly means are shipped on the split that crosses the month boundary. See :ref:`NRT runs <nrt>`.
...
(``APP_WILDFIRES_FWI``, ``APP_WILDFIRES_WISE``, ``APP_ENERGY_INDICATORS``,
...
``TAR_RUNDIR``
    If set to "True", compresses the SIM rundir. This should be set to False if the experiment does not contain a SIM job.

``TAR_LOGS``
    If set to "True", compresses the SIM logs into a tarball then deletes the raw log files. This should be set to False if the experiment does not contain a SIM job.
...
If a user wishes to reanalyze past LRA Generator data, they may disable this job by setting CONFIGURATION.ADDITIONAL_JOBS.LRA to "False" and designating a previous experiment to reanalyze using AQUA.PREV_EXP. Note that this experiment must be in the designated version of the AQUA catalog.

AQUA_ANALYSIS
~~~~~~~~~~~~~

Performs the analysis of the AQUA files. There is a default AQUA analysis configuration file but the user can designate a different one to use in the main.yml by setting the following:

.. code-block:: yaml

      AQUA:
        ANALYSIS_CONFIG: "%HPCROOTDIR%/.aqua/analysis/config.aqua-analysis.scenario.yaml"

Only the word before the .yaml needs to be changed. For example, if the storylines config is needed, the main.yml should have the following:

.. code-block:: yaml

      AQUA:
        ANALYSIS_CONFIG: "%HPCROOTDIR%/.aqua/analysis/config.aqua-analysis.storylines.yaml"
...
+---------------------------+-------------------------------+
| ``default``               | baseline-hist, everything else|
+---------------------------+-------------------------------+
...
The user can overwrite any parameter defining it in the ``main.yml`` file. This will have priority over the default configuration files loaded previously.
