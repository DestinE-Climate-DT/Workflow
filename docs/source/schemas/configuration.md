...
| IFS | `object` |  | object |  |  |  |  |
| IFS.USE_LOCAL_TIME | `string` |  | `False` `True` |  | `"False"` | If True, compute the IFS restart date offset using local time instead of UTC. Kept for backwards-compatibility with behaviour before v6.2.5. |  |
