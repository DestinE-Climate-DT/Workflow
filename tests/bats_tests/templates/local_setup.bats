...
@test "clean_restarts on but backup off" {
    ...
    run "${ROOTDIR}/proj/${PROJDEST}/templates/local_setup.sh" ${ROOTDIR} ${PROJDEST} ${MODEL_NAME} ${HPCARCH} ${MODEL_VERSION} ${ENVIRONMENT} ${DVC_INPUTS_BRANCH} ${APP} ${WORKFLOW} ${INSTALL} ${RUN_TYPE} ${COMPILE} ${USE_FXED_DVC_COMMIT} ${AQUA_ON} ${DOWNLOAD_ADDITIONAL_DEPENDENCIES} ${DATELIST} ${CLEAN_RESTARTS_ON} ${BACKUP_ON}
    [ "$status" -eq 1 ]
