...
setup() {
    ...
    export CHECK_NEGATIVE_SPACE="True"
    ...
    export SCRATCH_DIR="${ROOTDIR}/gpfs/scratch"
    export HPC_PROJECT_ROOT="${ROOTDIR}/gpfs/projects"
    export OPERATIONAL_PROJECT_SCRATCH="${SCRATCH_DIR}/testproject"
    export DEVELOPMENT_PROJECT_SCRATCH="${SCRATCH_DIR}/testproject"
...
@test "load_template_dqc" {
    run source "${ROOTDIR}/proj/${PROJDEST}/templates/dqc.sh" ${ROOTDIR} ${MODEL} ${CURRENT_ARCH} ${CHUNK_START_DATE} ${CHUNK_SECOND_TO_LAST_DATE} ${FDB_HOME} ${MEMBER} "${MEMBER_LIST}" ${CHECK_STANDARD_COMPLIANCE} ${CHECK_SPATIAL_COMPLETENESS} ${CHECK_SPATIAL_CONSISTENCY} ${CHECK_PHYSICAL_PLAUSIBILITY} ${CHECK_NEGATIVE_SPACE} ${EXPERIMENT} ${ACTIVITY} ${DQC_PROFILE_PATH} ${EXPVER} ${CLASS} ${GENERATION} ${HPC_CONTAINER_DIR} ${GSV_VERSION} ${LIBDIR} ${SCRIPTDIR} ${HPC_SCRATCH} ${FDB_INFO_FILE_PATH} ${FDB_INFO_FILE_NAME} ${CHUNK_END_DATE} ${BASE_VERSION} ${JOBNAME} ${OPERATIONAL_PROJECT_SCRATCH} ${DEVELOPMENT_PROJECT_SCRATCH} ${LOCAL_DIR} ${SCRATCH_DIR} ${HPC_PROJECT_ROOT}
    assert_success
}
