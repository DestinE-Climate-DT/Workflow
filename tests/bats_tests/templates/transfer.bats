...
mock_gribfile() {
    mkdir -p "${CURRENT_ROOTDIR}/transfer_requests"
    # The template names the file by realization, not by member: fc0 -> 1
    export MOCK_GRIB_FILE="${CURRENT_ROOTDIR}/transfer_requests/test_sdate_${SPLIT_START_DATE}_endate_${SECOND_TO_LAST_DATE}_real_${REALIZATION}.grb"
    dd if=/dev/zero of="${MOCK_GRIB_FILE}" bs=1M count=1
}
...
mock_fdb_copy_bins() {
    mkdir -p "${FDB_HOME}/bin"
    printf '#!/bin/bash\necho "class=d1,dataset=climate-dt"\n' > "${FDB_HOME}/bin/fdb-list"
    printf '#!/bin/bash\ntrue\n' > "${FDB_HOME}/bin/fdb-copy"
    chmod +x "${FDB_HOME}/bin/fdb-list" "${FDB_HOME}/bin/fdb-copy"
}
...
setup() {
    ...
    # cp -r "${PROJECT_DIR}/." "${CURRENT_ROOTDIR}/proj/${PROJDEST}"
    ...
    export DQC_PROFILE_PATH="${CURRENT_ROOTDIR}/profiles"
    export FDB_HOME="${CURRENT_ROOTDIR}/proj/${PROJDEST}/fdb"
    export EXPVER="test"
    export SCRATCH_DIR="${CURRENT_ROOTDIR}/proj/${PROJDEST}/scratch"
    export HPC_CONTAINER_DIR="${CURRENT_ROOTDIR}/proj/${PROJDEST}/containers"
    export GSV_VERSION="1"
    export LIBDIR="${CURRENT_ROOTDIR}/proj/${PROJDEST}/lib"
    export MEMBER="fc0"
    export MEMBER_LIST="fc0 fc1 fc2"
    export REALIZATION="1"
    export GENERATION="1"
    export SCRIPTDIR="${CURRENT_ROOTDIR}/proj/${PROJDEST}/runscripts"
    export FDB_PROD="${CURRENT_ROOTDIR}/proj/${PROJDEST}/fdb_prod"
    export DATABRIDGE_FDB_HOME="${CURRENT_ROOTDIR}/proj/${PROJDEST}/databridge_fdb"
    export FDB_INFO_FILE_PATH="${CURRENT_ROOTDIR}/proj/${PROJDEST}"
    ...
    export EXPID="a000"
    export SIM_START_DATE="20000101"
    export SPLIT_END_DATE="20000112"
    export DQC_PROFILE="lowres"
    export DATA_PORTFOLIO="production"
    export DQC_PROFILE_ROOT="${CURRENT_ROOTDIR}/profiles"
    export PROJECT="test"
    export USER="test"
    export CURRENT_CURRENT_ROOTDIR=${CURRENT_ROOTDIR}
    export MARS_BINARY="${FDB_HOME}/bin/mars"
    export CONTAINER_DIR="${HPC_CONTAINER_DIR}"
    export OPERATIONAL_PROJECT_SCRATCH="${SCRATCH_DIR}"
    export DEVELOPMENT_PROJECT_SCRATCH="${SCRATCH_DIR}"
    export MODIFY_METADATA="FALSE"
    export MODIFY_METADATA_FIELDS="none"
    export HPC_PROJECT_ROOT="${CURRENT_ROOTDIR}/proj"
    export LOCAL_DIR="${CURRENT_ROOTDIR}"
    export FDB_COPY_BIN="${FDB_HOME}/bin/fdb-copy"
    export FDB_CONFIG_HPC="${FDB_HOME}/config.yaml"
...
@test "load_transfer_template_working_lumi" {
    ...
    run source "${CURRENT_ROOTDIR}/proj/${PROJDEST}/templates/transfer.sh" ${CURRENT_ROOTDIR} ${CURRENT_ARCH} ${CHUNK} ${SPLIT_START_DATE} ${SECOND_TO_LAST_DATE} ${MODEL_NAME} ${EXPERIMENT} ${ACTIVITY} ${DQC_PROFILE_PATH} ${FDB_HOME} ${EXPVER} ${SCRATCH_DIR} ${HPC_CONTAINER_DIR} ${GSV_VERSION} ${LIBDIR} ${MEMBER} "${MEMBER_LIST}" ${GENERATION} ${SCRIPTDIR} ${FDB_PROD} ${DATABRIDGE_FDB_HOME} ${FDB_INFO_FILE_PATH} ${FDB_INFO_FILE_NAME} ${SPLIT_FIRST} ${SPLIT_SECOND_TO_LAST_DATE} ${CHUNK_SECOND_TO_LAST_DATE} ${SPLITS} ${EXPID} ${SIM_START_DATE} ${SPLIT_END_DATE} ${CONTAINER_COMMAND} ${DQC_PROFILE} ${DATA_PORTFOLIO} ${DQC_PROFILE_ROOT} ${PROJECT} ${USER} ${CURRENT_CURRENT_ROOTDIR} ${MARS_BINARY} ${PROJDEST} ${DATABRIDGE_DATABASE}
    assert_success
    # check to see if the grib file was deleted
    [ ! -f "${MOCK_GRIB_FILE}" ]
}
...
@test "load_transfer_template_working_MN5_transfer" {
    ...
    run source "${CURRENT_ROOTDIR}/proj/${PROJDEST}/templates/transfer.sh" ${CURRENT_ROOTDIR} ${CURRENT_ARCH} ${CHUNK} ${SPLIT_START_DATE} ${SECOND_TO_LAST_DATE} ${MODEL_NAME} ${EXPERIMENT} ${ACTIVITY} ${DQC_PROFILE_PATH} ${FDB_HOME} ${EXPVER} ${SCRATCH_DIR} ${HPC_CONTAINER_DIR} ${GSV_VERSION} ${LIBDIR} ${MEMBER} "${MEMBER_LIST}" ${GENERATION} ${SCRIPTDIR} ${FDB_PROD} ${DATABRIDGE_FDB_HOME} ${FDB_INFO_FILE_PATH} ${FDB_INFO_FILE_NAME} ${SPLIT_FIRST} ${SPLIT_SECOND_TO_LAST_DATE} ${SPLITS} ${EXPID} ${SIM_START_DATE} ${SPLIT_END_DATE} ${CONTAINER_COMMAND} ${DQC_PROFILE} ${DATA_PORTFOLIO} ${DQC_PROFILE_ROOT} ${PROJECT} ${USER} ${CURRENT_CURRENT_ROOTDIR} ${MARS_BINARY} ${PROJDEST} ${DATABRIDGE_DATABASE} ${CONTAINER_DIR} ${OPERATIONAL_PROJECT_SCRATCH} ${DEVELOPMENT_PROJECT_SCRATCH} ${MODIFY_METADATA} ${MODIFY_METADATA_FIELDS} ${HPC_PROJECT_ROOT} ${LOCAL_DIR} ${FDB_COPY_BIN} ${FDB_CONFIG_HPC}
    assert_success
}
