#!/bin/bash

# Utils functions for remote_setup.sh

function post_compilation_ifs-nemo() {
    true
}

function post_compilation_icon() {
    true
}

function post_compilation_nemo() {
    true
}

###################################################
# Prints the commits and the versions of the
# used modules in ifs-bundle
###################################################
function print_rev {
    _prev_dir=$PWD
    cd "$1" && echo "$1" "$(git rev-parse HEAD)" "$(git rev-parse --symbolic-full-name HEAD)" "$(git --no-pager tag --points-at HEAD)"
    cd "$_prev_dir"
}

###################################################
# Prints the commits and the versions of the
# used modules in ifs-bundle
###################################################
function print_rev_all {
    echo "path" "commit" "ref" "tag"
    echo "----" "------" "---" "---"
    for d in $(find source -maxdepth 1 -type d); do print_rev "$d"; done
}

#####################################################
# Compiles IFS-Nemo model using RAPS
# Globals:
#   INSTALL_DIR
# Arguments:
#   arch
#   add_flags
######################################################
function compile_ifs-nemo() {

    arch=$1
    add_flags=${2:-""}

    cd "${MODEL_PATH}"/
    ./ifs-bundle create -j4 --shallow

    # Start the bundle build process
    if [ ! -d "${BUNDLE_BUILD_DIR}" ]; then
        ./ifs-bundle build --arch ${arch} --with-single-precision \
            --with-double-precision-nemo --nemo-version=V40 \
            --nemo-grid-config=eORCA1_GO8_Z75 --nemo-ice-config=SI3 \
            --with-multio-for-nemo-sglexe --dry-run \
            --nemovar-grid-config=ORCA1_Z42 --nemovar-ver=DEV ${add_flags}

        cd ${BUNDLE_BUILD_DIR}
        ./configure.sh
    fi

    if [ ! -f "${BUNDLE_BUILD_DIR}/bin/ifsMASTER.SP" ]; then
        cd "${BUNDLE_BUILD_DIR}"
        source env.sh
        make -j 20 VERBOSE=1 | tee raps_make.log

    fi

    if [ -f "${BUNDLE_BUILD_DIR}/bin/ifsMASTER.SP" ]; then
        echo "Compilation sucessful"
    else
        echo "Compilation failed. There is no ifsMASTER.SP"
        exit 1
    fi

    cd "${BUNDLE_SOURCE_DIR}/raps"
    set +e
    source initbm
    set -e

    # Print the versions of the used modules
    cd "${MODEL_PATH}"
    print_rev_all | column -t >"bundle_versions"

}

#######################################################
# Function to generate the NEMO environment file
# Currently unused.
# Globals:
#   ARCH_NAME
#   NEMO_NETCDF_FORTRAN_PATH
#   NEMO_NETCDF_C_PATH
#   NEMO_HDF5_PATH
#   NEMO_XIOS_PATH
#   NEMO_CPP
#   NEMO_CC
#   NEMO_FC
#   NEMO_FCFLAGS
#   NEMO_LDFLAGS
#   NEMO_FPPFLAGS
# Arguments:
#######################################################
function generate_nemo_env_file() {
    cat >arch/arch-$ARCH_NAME.fcm <<EOF
%NCDF_INC            -I$NEMO_NETCDF_FORTRAN_PATH/include
%NCDF_LIB            -L$NEMO_NETCDF_FORTRAN_PATH/lib -L$NEMO_NETCDF_C_PATH/lib -L$NEMO_HDF5_PATH/lib -lhdf5 -lhdf5_hl -lnetcdf -lnetcdff
%XIOS_INC            -I$NEMO_XIOS_PATH/inc
%XIOS_LIB            -L$NEMO_XIOS_PATH/lib -lxios -lstdc++
%CPP                 $NEMO_CPP
%CC                  $NEMO_CC
%FC                  $NEMO_FC
%FCFLAGS             $NEMO_FCFLAGS
%FFLAGS              %FCFLAGS
%LD                  %FC
%LDFLAGS             $NEMO_LDFLAGS
%FPPFLAGS            $NEMO_FPPFLAGS
%AR                  ar
%ARFLAGS             rs
%MK                  gmake
%USER_INC            %NCDF_INC %XIOS_INC
%USER_LIB            %NCDF_LIB %XIOS_LIB
EOF

}

#####################################################
# Stops the execution when calling the compilation of a model, if it is not available.
# Globals:
# Arguments:
#####################################################
function current_model_compile_not_available() {
    echo "Compilation of ${MODEL_NAME^^} is currently not available with the workflow. Please use a pre-compiled version of the model by setting MODEL.COMPILE to False and designating a pre-compiled version with MODEL.VERSION."
    exit 1
}

######################################################
# Function to compile NEMO standalone.
# Currently unused.
# Globals:
# Arguments:
######################################################
function compile_nemo() {
    current_model_compile_not_available
}

#####################################################
# Compiles IFS-Fesom model using RAPS
# Currently unused.
# Globals:
# Arguments:
######################################################
function compile_ifs-fesom() {
    current_model_compile_not_available
}

######################################################
# Compiles the ICON model differentiating between PU's
# Currently unused.
# Globals:
# Arguments:
######################################################
function compile_icon() {
    current_model_compile_not_available
}

############################################
# Loads the fdb config for ifs-based models
# Globals:
#   MODEL_VERSION
#   HPCROOTDIR
#   PROJDEST
#   MODEL_NAME
#   MODEL_PATH
#   FDB_HOME
# Arguments:
#   None
############################################
function load_fdb_ifs() {
    # Directory definition
    if [ -z "${MODEL_VERSION}" ]; then
        RAPS_BIN="${HPCROOTDIR}/${PROJDEST}/${MODEL_NAME}/source/raps/bin"
    else
        RAPS_BIN="${MODEL_PATH}/source/raps/bin"
    fi

    mkdir -p ${FDB_HOME}
    cd ${FDB_HOME}
    FDB_DIRS=("native" "." "latlon")

    for FDB in "${FDB_DIRS[@]}"; do
        FDB_DIR_PATH="${FDB_HOME}/${FDB}"
        mkdir -p "${FDB_DIR_PATH}/etc/fdb"
        chmod -R 750 "${FDB_DIR_PATH}"

        FDB_prefix="${FDB//./}"

        cp "${HPCROOTDIR}/LOG_${EXPID}/config${FDB_prefix,,}_${SIM_START_DATE}_REMOTE_SETUP" "${FDB_DIR_PATH}/etc/fdb/config.yaml"
        cp "${SCRIPTDIR}/FDB/schema" "${FDB_DIR_PATH}/etc/fdb/schema"
    done
}

#####################################################
# Loads the fdb config for icon-based models
# Globals:
#   HPCROOTDIR
#   RUN_TYPE
#   FDB_DIR
#   CHUNK_START_DATE
# Arguments:
######################################################
function load_fdb_icon() {

    mkdir -p "${FDB_HOME}/etc/fdb"

    cp "${HPCROOTDIR}/LOG_${EXPID}/config_${SIM_START_DATE}_REMOTE_SETUP" "${FDB_HOME}/etc/fdb/config.yaml"
    cp "${SCRIPTDIR}/FDB/schema" "${FDB_HOME}/etc/fdb/schema"
}

############################################################
# Function to install AQUA
# Globals:
#   HPCARCH-short
#   HPCROOTDIR
#   PROJDEST
#   CATALOG_NAME
#   AQUA
#   EXPID
#   EXPVER
#   MODEL
#   DATA_PORTFOLIO
#   START_DATE
#   AQUA_CONTAINER
# Arguments:
#   AQUA_CONFIG, GRID_BUILD_ENABLED, AQUA_GRID_ATM, MODEL_NAME_LOWER,
#   DQC_PROFILE, GRID_OCE, RESOLUTION, REALIZATIONS, EXPERIMENT_NAME
############################################################
function install_aqua() {
    AQUA_CONFIG=$1
    GRID_BUILD_ENABLED=${2:-"False"}
    AQUA_GRID_ATM=${3:-""}
    MODEL_NAME_LOWER=${4:-""}
    DQC_PROFILE=${5:-""}
    GRID_OCE=${6:-""}
    RESOLUTION=$7
    REALIZATIONS=${8:-""}
    EXPERIMENT_NAME=$9

    if [ -d "${AQUA_CONFIG}" ]; then
        mv "${AQUA_CONFIG}" "${AQUA_CONFIG}_$(date "+%Y%m%d%H%M%S")"
    fi
    # If AQUA_START_DATE is not set, data_start_date will be null and needs to be substituted
    if [ -z "${AQUA_START_DATE}" ]; then
        sed -i "s/^data_start_date: .*/data_start_date: \"${SIM_START_DATE}\"/" \
            "${HPCROOTDIR}/LOG_${EXPID}/config_catalog_${SIM_START_DATE}_AQUA_SETUP"
    fi

    # The AQUA model name carries the atmospheric resolution in km
    # (e.g. IFS-NEMO-120km). catgen (AQUA >= 0.19.12) is given this
    # resolution-suffixed name via config_catalog.yaml and writes the entry
    # straight into catalog/<MODEL_AND_RES>/, which is what the catalog and the
    # AQUA readers (analysis, drop, grids build) address.
    MODEL_AND_RES="${MODEL_NAME_UPPER}-${RESOLUTION}"
    singularity exec \
        --cleanenv \
        --env PYTHONPATH=/opt/conda/lib/python3.10/site-packages \
        --env ESMFMKFILE=/opt/conda/lib/esmf.mk \
        --env PYTHONPATH=$AQUA \
        --env AQUA=$AQUA \
        --env AQUA_REGENCAT=$AQUA_REGENCAT \
        --env HPCARCH_short=$HPCARCH_short \
        --env HPCROOTDIR=$HPCROOTDIR \
        --env PROJDEST=$PROJDEST \
        --env CATALOG_NAME=$CATALOG_NAME \
        --env EXPID=$EXPID \
        --env EXPVER=$EXPVER \
        --env EXPERIMENT_NAME=$EXPERIMENT_NAME \
        --env MODEL_NAME_UPPER=$MODEL_NAME_UPPER \
        --env MODEL_AND_RES=$MODEL_AND_RES \
        --env DATA_PORTFOLIO=$DATA_PORTFOLIO \
        --env START_DATE=$SIM_START_DATE \
        --env REALIZATIONS="$REALIZATIONS" \
        --env AUTHOR=$USER \
        --env PYTHONUSERBASE=1 \
        --env AQUA_CONFIG=$AQUA_CONFIG \
        --env AQUA_START_DATE=$AQUA_START_DATE \
        --env GRID_BUILD_ENABLED=$GRID_BUILD_ENABLED \
        --env AQUA_GRID_ATM=$AQUA_GRID_ATM \
        --env MODEL_NAME_LOWER=$MODEL_NAME_LOWER \
        --env DQC_PROFILE=$DQC_PROFILE \
        --env GRID_OCE=$GRID_OCE \
        --bind=${HPCROOTDIR} \
        $AQUA_CONTAINER \
        bash -c \
        '
    set -xuve

    # Install AQUA
    yes n | aqua install ${HPCARCH_short} -p "${AQUA_CONFIG}"
    export AQUA_CONFIG=${AQUA_CONFIG}

    # Configure grids path only if AQUA_GRID_BUILD is enabled
    # Update catalog machine.yaml to use experiment-local grids
    GRID_BUILD_ENABLED="${GRID_BUILD_ENABLED:-False}"
    if [ "${GRID_BUILD_ENABLED,,}" = "true" ]; then
        echo "AQUA_GRID_BUILD enabled: Will configure catalog to use ${AQUA_CONFIG}/grids"
    else
        echo "AQUA_GRID_BUILD disabled: Using system default grids path"
    fi

    # Install the working catalog
    if [ ! -d "${AQUA_CONFIG}/catalogs/${CATALOG_NAME}" ]; then
        aqua add "${CATALOG_NAME}" -e "${HPCROOTDIR}/catalog/catalogs/${CATALOG_NAME}"
    else
        # its a re-run
	    echo "${CATALOG_NAME} is already installed"
    fi

    # Update catalog YAML files when AQUA_GRID_BUILD enabled
    # Uses standalone Python script instead of inline python3 -c for readability
    if [ "${GRID_BUILD_ENABLED,,}" = "true" ]; then
        AQUA_CONFIG_SCRIPT="${HPCROOTDIR}/${PROJDEST}/runscripts/aqua/update_aqua_config.py"
        GRID_DATADIR="${HPCROOTDIR}/aqua-data"

        # Update machine.yaml: redirect grids/weights/areas paths to experiment-local directory
        MACHINE_YAML="${AQUA_CONFIG}/catalogs/${CATALOG_NAME}/machine.yaml"
        if [ -f "${MACHINE_YAML}" ]; then
            python3 "${AQUA_CONFIG_SCRIPT}" update-paths \
                --machine_yaml "${MACHINE_YAML}" \
                --platform "${HPCARCH_short}" \
                --data_dir "${GRID_DATADIR}"
        fi

        # Update matching_grids.yaml: set atm/ocean grid for this model and profile
        MATCHING_GRIDS="${AQUA_CONFIG}/catgen/matching_grids.yaml"
        if [ -f "${MATCHING_GRIDS}" ] && [ -n "${AQUA_GRID_ATM}" ] && [ -n "${MODEL_NAME_LOWER}" ] && [ -n "${DQC_PROFILE}" ]; then
            python3 "${AQUA_CONFIG_SCRIPT}" update-grids \
                --matching_grids_yaml "${MATCHING_GRIDS}" \
                --model "${MODEL_NAME_LOWER}" \
                --profile "${DQC_PROFILE}" \
                --atm_grid "${AQUA_GRID_ATM}" \
                --ocean_grid "${GRID_OCE}"
        fi
    fi

    # Install the obs catalog
    if [ ! -d "${AQUA_CONFIG}/catalogs/obs" ]; then
        aqua add obs -e "${HPCROOTDIR}/catalog/catalogs/obs"
    else
        echo "Obs catalog is already installed"
    fi

    CATALOG_DIR="${AQUA_CONFIG}/catalogs/${CATALOG_NAME}/catalog/${MODEL_AND_RES}/"
    # catgen names the entry after its exp key -- the experiment name, not the
    # expver. Run types that pin an expver would never find the entry.
    CATALOG_FILE="${HPCROOTDIR}/catalog/catalogs/${CATALOG_NAME}/catalog/${MODEL_AND_RES}/${EXPERIMENT_NAME}.yaml"

    # Create catalog directory with -p (if it doesnt exist)
    mkdir -p ${CATALOG_DIR}

    # Regenerate depending on regen_cat key
    if [ "${AQUA_REGENCAT,,}" = "true" ] || [ ! -f "${CATALOG_FILE}" ]; then
        # Remove existing catalog if regenerating
        if [ "${AQUA_REGENCAT,,}" = "true" ] && [ -f "${CATALOG_FILE}" ]; then
            echo "Removing existing catalog file as key REGENERATE_CATALOGS is TRUE."
            rm "${CATALOG_FILE}"
        fi

        echo "Generating catalog..."
        aqua catgen -p "${DATA_PORTFOLIO}" -c "${HPCROOTDIR}/LOG_${EXPID}/config_catalog_${START_DATE}_AQUA_SETUP"

    else
        echo "The catalog entry for your experiment already exists."
    fi

    # catgen only knows the member count, so it writes 1..N; set the real realizations
    if [ -n "${REALIZATIONS}" ] && [ -f "${CATALOG_FILE}" ]; then
        python3 "${HPCROOTDIR}/${PROJDEST}/runscripts/aqua/update_aqua_config.py" \
            update-realizations --catalog_file "${CATALOG_FILE}" --realizations "${REALIZATIONS}"
    fi
    '
}

#####################################################
# Function to check out the inputs
# Globals:
#    HPCROOTDIR
#    PROJDEST
# Arguments:
#    DVC_INPUTS_CACHE
#####################################################
function inputs_dvc_checkout() {
    DVC_PATH="$HPCROOTDIR/$PROJDEST/dvc-cache-de340"
    cache_dir=$1
    singularity exec \
        --cleanenv --no-home --bind "$DVC_PATH" \
        --bind "$cache_dir" --env cache_dir="$cache_dir" \
        --env HPC_CONTAINER_DIR="$HPC_CONTAINER_DIR" \
        --env DVC_VERSION="$DVC_VERSION" \
        --env DVC_PATH="$DVC_PATH" \
        "${HPC_CONTAINER_DIR}"/dvc/dvc_${DVC_VERSION}.sif \
        bash -c \
        "set -xuve && cd ${DVC_PATH} && dvc config cache.dir ${cache_dir} && dvc checkout"
}

#####################################################
# Selects the arch based on the PU
# Globals:
#   None
# Arguments:
#   PU
#   ARCH_CPU
#   ARCH_GPU
#####################################################
function get_arch_compilation_flags() {
    # Input
    PU=$1
    ARCH_CPU="$2"
    ARCH_GPU="$3"
    ADDITIONAL_COMPILATION_FLAGS_CPU="$4"
    ADDITIONAL_COMPILATION_FLAGS_GPU="$5"

    case $PU in
    cpu)
        arch=$ARCH_CPU
        add_flags=${ADDITIONAL_COMPILATION_FLAGS_CPU:-""}
        ;;
    gpu)
        arch=$ARCH_GPU
        add_flags=${ADDITIONAL_COMPILATION_FLAGS_GPU:-""}
        ;;
    *)
        echo "ERROR: Unknown PU=$PU in $0::${FUNCNAME[0]}"
        exit 1
        ;;
    esac

    # Return
    echo "$arch" "$add_flags"
}
