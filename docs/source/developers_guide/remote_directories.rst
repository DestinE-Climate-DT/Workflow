...
- ``restarts``
- ``run_<CHUNK_STARTDATE>-<CHUNK_ENDDATE>``

The restarts folder contains the restart files for each chunk, this is discussed in more detail in section :ref:`restarts`.
For the run directories, in the case for ICON it contains the model namelists for that chunk, links to the necessary input files, and more model specific files that are needed for the model to run. The run directories are named after the chunk start and end date, for example: ``run_19900101-19900201``.
...
- ``wipe_requests``: Contains cleanup/wipe request files
