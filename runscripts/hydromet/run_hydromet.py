...
from HydroMet.create_config import update_template
from HydroMet.create_config import update_yaml_from_txt
...
parser.add_argument(
    ...
    nargs="?",
    type=bool,
    help="Switch to activate extra kostra run",
    default=False,
)

parser.add_argument(
    "precip", nargs="?", type=str, help="Name of precipitation variable", default=False
)
...
def process_netcdf_files(
    ...
    for file in files:
        ...
        if start <= file_date <= end:
            run_WetCatO.run_WetCat_object_detection(config_dir, file)

    run_WetCatE.run_WetCat_event_detection(config_dir)
    object_count, files = check_directory(WetCat_out + "/objects", ".csv")
    if os.path.isdir(WetCat_out + "processed_output/") and object_count > 11:
        files = sorted(files)
        for i in files[:-5]:
            shutil.move(
                WetCat_out + "/objects/" + i,
                WetCat_out + "processed_output/" + i,
            )
...
def run_hydromet(
    ...
    kostra : bool, optional
        Switch to activate a kostradmaus run after a successful WetCat run.
    kostra : bool, optional
        Name of precipitation variable.
    """
    if precip == "tp":
        update_template("data_var", "tp")
        update_template("rain_unit", "m")

    prudence = config_dir[-6:-4]
    update_yaml_from_txt(
        prudence + ".txt", "template_config_hydromet_" + prudence + ".yml"
    )
    update_template(
        "catalogue_dir",
        "WetCat_out" + "_" + prudence + "/",
        "template_config_hydromet_" + prudence + ".yml",
    )
            ...
            if iniyear and inimonth and iniday:
                if iniday == finday:
                    ...
                    if match:
                        ...
                        if (
                            os.path.isdir(WetCat_out + "processed_output/")
                            and object_count > 11
                        ):
                            files = sorted(files)
                            for i in files[:-1]:
                        ...
                        raise Exception("Could not find DestinE data")
                else:
