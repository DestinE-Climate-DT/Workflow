...
def first_date_greater(date1: str, date2: str) -> bool:
    """
    Compare two date strings in the format YYYY-MM-DD.
    Return True if date1 is greater than date2, False otherwise.
    ...
    for key, value in updates.items():
        ...
        if value is not None:
            ...
            if key.endswith("date") and value:
                ...
                current_value = updated_data.get(section, {}).get(sub_key)
                # Only update if new date is greater than current one
                if current_value is None or first_date_greater(value, current_value):
