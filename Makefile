...
.PHONY: docs-latexpdf
docs-latexpdf:
	sed -i 's/✅/✔/g' docs/source/schemas/*.md;
	sudo docker run --rm -it -v "$(shell pwd):/code" -w /code sphinxdoc/sphinx-latexpdf bash -c "sed -i 's/✅/✔/g' docs/source/schemas/*.md; pip install --upgrade pip; pip install .[docs] && cd docs && make latexpdf"
	sed -i 's/✔/✅/g' docs/source/schemas/*.md;
