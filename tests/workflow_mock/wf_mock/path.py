...
class Path(object):
    ...
    def make(self, override=False, directory=False, ignore_errors=False):
        ...
        if exist and not override:
            msg = 'The path "{}" already exist and will not be created.'.format(
                self.path
            )
            if ignore_errors:
    ...
    def __copy_file(self, des: str, source=None):
        ...
        try:
            ...
            logger.error(
                "Failed to copy a file from {} to {}. {}".format(source, des, str(exp))
            )
    ...
    def copy_to(self, destination):
        ...
        if not self.exists():
            logger.error(
                f"This path {self.path} does not exist and cannot be copied to {destination}",
                verbose=0,
            )
            return False
    ...
    def unzip(self, output_dir):
        ...
        try:
            ...
            logger.critical(
                f"Failed to extract {self.path} to {output_dir}. Error : {e}", verbose=0
            )
...
def copy_folder_with_permissions(root_dir: Path, source_dir: Path, target_dir: Path):
    ...
    def walk_callback(root, subdirs, files):
        for file in files:
            ...
            with open(file_path.path, "rb") as f:
                lib_folder_content[relative_path] = (
                    file_path.state().st_mode,
                    f.read(),
                )
                f.close()
    ...
    for file in lib_folder_content.keys():
        ...
        Path(file_path.dirname()).make(
            override=False, ignore_errors=True, directory=True
        )
