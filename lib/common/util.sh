...
#####################################################
# Computes the member number from the member name:
# fcN -> realization N+1 (fc10 -> 11).
# Globals:
# Arguments:
#    MEMBERS_LIST
#    MEMBER
...
function get_member_number() {
    ...
    # Check the MEMBER belongs to the experiment's member list
    MEMBER_FOUND=false
    for CANDIDATE in "${MEMBERS_ARRAY[@]}"; do
    ...
    # Errors go to stderr so $(get_member_number ...) does not capture them
    if [ "${MEMBER_FOUND}" == false ]; then
        echo "Member not found in the list." >&2
        exit 1
    fi

    if [[ "$MEMBER" =~ ^fc([0-9]+)$ ]]; then
        # 10# keeps fc08/fc09 out of octal
        echo $((10#${BASH_REMATCH[1]} + 1))
    else
        echo "Member '${MEMBER}' is not in the expected fcN format." >&2
        exit 1
    fi
}

#####################################################
# Maps every member of a list to its realization.
# Globals:
# Arguments:
#    MEMBERS_LIST
#
#####################################################
function get_realization_list() {
    MEMBERS_LIST=$1
    REALIZATION_LIST=""

    for ENS_MEMBER in ${MEMBERS_LIST}; do
        REALIZATION_LIST+="$(get_member_number "${MEMBERS_LIST}" "${ENS_MEMBER}") " || exit 1
    done

    echo "${REALIZATION_LIST}" | xargs
}
...
#####################################################
# Detect if a split crosses a month boundary (NRT daily splits).
# NRT transfers run per daily split, while the month is still being
# simulated. Monthly means only reach the FDB once the month is
# complete, so monthly streams must be processed by the split covering
# the last day of the month instead of the one containing day 1
# (which is what enable_process_monthly selects).
# Arguments:
#   START_DATE
#   END_DATE (exclusive split end date)
######################################################
function requires_process_monthly_nrt() {
    local start_date="$1"
    local end_date="$2"
...
#####################################################
# Get the first day of the month of a given date (YYYYMM01).
# Monthly streams are keyed by month/year in the FDB, and the request
# generators only emit those keys when day 1 of the month is inside the
# requested date range. NRT boundary splits start on the last day of
# the month, so their monthly requests must be widened to start on
# day 1 (see requires_process_monthly_nrt).
# Arguments:
#   DATE (YYYYMMDD)
######################################################
...
#####################################################
# Resolve the request start date for a transfer/DQC profile.
# Monthly-mean profiles (matched by *monthly* on LUMI, *clmn* on MN5)
# must be widened to day 1 of the month, because NRT boundary splits
# start on the last day; every other profile keeps the split start date.
# Arguments:
#   PROFILE_FILE
#   START_DATE (YYYYMMDD)
######################################################
function get_request_start_date() {
    ...
    if [[ "$profile_file" == *monthly* || "$profile_file" == *clmn* ]]; then
        # lib/common/util.sh (get_month_start_date) (auto generated comment)
        get_month_start_date "$start_date"
    else
        echo "$start_date"
    fi
}
...
function purge_duplicated_data() {
    ...
    ${CONTAINER_COMMAND} exec --cleanenv --no-home \
        ...
        --env "EXPVER=${EXPVER}" \
        --env "START_DATE=${REQUEST_START_DATE:-${START_DATE}}" \
        --env "CHUNK=${CHUNK}" \
...
#####################################################
# Checks if the profiles exist already. If not, calls generate_profiles.
# Globals:
#   CURRENT_ROOTDIR
######################################################
function check_profiles_exist() {
    if [ ! -d "${CURRENT_ROOTDIR}/profiles" ]; then
        generate_profiles
    fi
}
