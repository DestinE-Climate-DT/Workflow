...
mock_pdfs() {
    echo 'Mocking PDFs'
    mkdir -p $1/aqua-analysis/$2/$3/$4/r$5
    touch $1/aqua-analysis/$2/$3/$4/r$5/plot1.pdf
    true
}
...
@test "load_template_without_mocked_pdfs" {
    run source "${ROOTDIR}/proj/${PROJDEST}/templates/aqua/aqua_analysis.sh" "${ROOTDIR}" "${PROJDEST}" "${EXPID}" "${HPC_PROJECT}" "${CONTAINER_VERSION}" "${MODEL}" "${EXPVER}" "${EXPERIMENT_NAME}" "${CATALOG_NAME}" "${APP_OUTPATH}" "${CURRENT_ARCH}" "${HPC_SCRATCH}" "${AQUA_SEARCH_TIME}"
    assert_failure
}

@test "load_template_with_mocked_pdfs" {
    export RUN_LRA_GENERATOR="True"
    mock_pdfs ${APP_OUTPATH} ${CATALOG_NAME} ${MODEL} ${EXPERIMENT_NAME} ${REALIZATION}
    run source "${ROOTDIR}/proj/${PROJDEST}/templates/aqua/aqua_analysis.sh" "${ROOTDIR}" "${PROJDEST}" "${EXPID}" "${HPC_PROJECT}" "${CONTAINER_VERSION}" "${MODEL}" "${EXPVER}" "${EXPERIMENT_NAME}" "${CATALOG_NAME}" "${APP_OUTPATH}" "${CURRENT_ARCH}" "${HPC_SCRATCH}" "${AQUA_SEARCH_TIME}" "${RUN_LRA_GENERATOR}"
    assert_success
}
...
@test "load_template_with_mocked_pdfs_lra_generator_false" {
    ...
    PREV_AQUA_EXP="t001"
    mock_pdfs ${APP_OUTPATH} ${CATALOG_NAME} ${MODEL} ${PREV_AQUA_EXP} ${REALIZATION}
    run source "${ROOTDIR}/proj/${PROJDEST}/templates/aqua/aqua_analysis.sh" "${ROOTDIR}" "${PROJDEST}" "${EXPID}" "${HPC_PROJECT}" "${CONTAINER_VERSION}" "${MODEL}" "${EXPVER}" "${EXPERIMENT_NAME}" "${CATALOG_NAME}" "${APP_OUTPATH}" "${CURRENT_ARCH}" "${HPC_SCRATCH}" "${AQUA_SEARCH_TIME}" "${RUN_LRA_GENERATOR}" "${PREV_AQUA_EXP}"
    assert_success
}
