...
# Check if the compile flag is set to True and stops if the model compilation is not supported.
if [ "${COMPILE}" == "True" ]; then
    # lib/common/checkers.sh (checker_compile) (auto generated comment)
    checker_compile
fi
...
# Check that every member maps to its own realization
# lib/common/checkers.sh (checker_members) (auto generated comment)
